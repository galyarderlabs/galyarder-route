...
### ✨ New Features
- **feat(docs):** integrate multi-page documentation into Galyarder Route dashboard (#1969)
...
**The following contributors had their code and ideas integrated across multiple releases without proper merge credit. Thank you for your invaluable contributions to Galyarder Route:**
...
- **fix(antigravity):** scrub internal Galyarder Route headers (#1794)
...
- **feat(cli):** Add `galyarder-route reset-encrypted-columns` recovery command — nulls encrypted credential columns (`api_key`, `access_token`, `refresh_token`, `id_token`) in `provider_connections` while preserving provider metadata, giving users affected by #1622 a clean recovery path without losing configurations.
...
- **fix(cli-tools):** Preserve existing OpenCode configuration (MCP servers, custom providers, comments) when saving Galyarder Route settings — uses `jsonc-parser` for tree-preserving edits instead of destructive JSON roundtrip. Fix API key clipboard copy to use raw keys instead of masked placeholders. Add theme-aware OpenCode light/dark SVG logos (#1626 — thanks @JasonLandbridge).
...
- **docs(env):** Add `GALYARDER_ROUTE_ALLOW_PRIVATE_PROVIDER_URLS` to `.env.example` with documentation for LM Studio and other local provider use cases (#1623).
...
- **feat(mcp):** Register `galyarder_route_cache_stats` and `galyarder_route_cache_flush` across MCP schemas, server registration, handlers, docs, and tests.
...
- **feat(providers):** Mark Qwen OAuth provider as deprecated following the upstream free tier shutdown on 2026-04-15. Adds deprecation warning to CLI tool UI and rewrites `saveQwenConfig` to inject Galyarder Route as a multi-provider (openai, anthropic, gemini) via `.qwen/settings.json` and `.qwen/.env` (#1437)
...
- **fix(dashboard):** Cleaned up target codex `config.toml` structure preventing recursive section rendering by enforcing quotes on section dot paths and mapping correct UI `GALYARDER_ROUTE_API_KEY` names.
- **fix(mcp):** Add dedicated explicit timeout constraint overrides for search handlers (#1280)
- **fix(crypto):** Add validation guard to encryption layer to surface clear UI errors when cryptographic environment variables are missing, replacing raw Node.js TypeErrors. Legacy env vars `GALYARDER_ROUTE_CRYPT_KEY` and `GALYARDER_ROUTE_API_KEY_BASE64` now also accepted as fallbacks (#1165)
...
- **Database Health Check System:** Automated periodic SQLite integrity monitoring via `runDbHealthCheck()` — detects orphan quota/domain rows, broken combo references, stale snapshots, and invalid JSON state. Runs every 6 hours (configurable via `GALYARDER_ROUTE_DB_HEALTHCHECK_INTERVAL_MS`), with auto-repair and pre-repair backup. Exposed as **MCP tool #18** (`galyarder_route_db_health_check`) with Zod schemas and `autoRepair` option. Dashboard panel in Health page with status card, issue count, repaired count, and one-click repair button
...
- **E2E Auth Bypass:** Added `NEXT_PUBLIC_GALYARDER_ROUTE_E2E_MODE` environment flag to bypass authentication gates for dashboard and management API routes during Playwright E2E test runs
...
- **API Key Secret Hardening:** Removed the hardcoded `"galyarder-route-default-insecure-api-key-secret"` fallback from `apiKey.ts` — the function now fails fast if `API_KEY_SECRET` is unset, relying on the startup validator to auto-generate it
...
- **Desktop Windows Build Crash:** Fixed `better_sqlite3.node is not a valid Win32 application` preventing Galyarder Route Desktop from launching on Windows by properly removing the ABI-mismatched sqlite cache from Next.js standalone and falling back to the cross-compiled Electron equivalent during packager build steps (#1163)
- **Login Visual Security:** Removed the raw fallback hash dump that artificially rendered underneath the login modal in Docker instances missing `GALYARDER_ROUTE_API_KEY_BASE64` flags (#1148)
...
- **Ecosystem Proxies:** Completed explicit integration mapping model synchronization schedulers, OAuth cycles, and Token Check refreshes safely through Galyarder Route's native system upstream proxies (#953).
- **MCP Extensibility:** Added and successfully registered the new `galyarder_route_web_search` MCP framework tool out of beta into production schemas (#951).
...
- **Cloudflare Quick Tunnels State:** Fixed a severe state inconsistency where restarted Galyarder Route instances erroneously showed destroyed tunnels as active, and defaulted cloudflared tunneling to `HTTP/2` to eliminate UDP receive buffer log spam (#925).
...
- **Web Search MCP:** Added the `galyarder_route_web_search` tool with the `execute:search` scope.
...
> On the first startup after upgrading, Galyarder Route archives legacy request logs from `DATA_DIR/logs/`, legacy `DATA_DIR/call_logs/`, and `DATA_DIR/log.txt` into `DATA_DIR/log_archives/*.zip`, then removes the deprecated layout and switches to the new unified artifact format under `DATA_DIR/call_logs/`.
...
- **Dashboard UI Restoration & Consolidation:** Resolved sidebar logic omitting the Debug section, and cleared Nextjs routing warnings by moving standalone `/dashboard/mcp` and `/dashboard/a2a` pages explicitly into embedded Route Gateway UI components.
...
- **SQLite Backup Control:** New `GALYARDER_ROUTE_DISABLE_AUTO_BACKUP` env flag to disable automatic SQLite backups (#846)
...
- **Admin Updater:** Dynamic source-installation detection for the dashboard Updater. Safely disables the `Update Now` button when Galyarder Route is built locally rather than through npm, prompting for `git pull` (#743).
...
- **Void Linux Deployment Support (#732)** — Integrated `xbps-src` packaging template and instructions to natively compile and install Galyarder Route with `better-sqlite3` bindings via cross-compilation target.
...
- **Global Fallback Provider (#689)** — When all combo models are exhausted (502/503), Galyarder Route now attempts a configurable global fallback model before returning the error. Set `globalFallbackModel` in settings to enable.
...
- **Fix #708** — Prevented token bleeding for Claude Code users routing through Galyarder Route by correctly preserving native \`cache_control\` headers during Claude-to-Claude passthrough (PR #708 by @tombii)
...
- **Claude Prompt Caching Passthrough** — Fixed cache_control markers being stripped in Claude passthrough mode (Claude → Galyarder Route → Claude), which caused Claude Code users to deplete their Anthropic API quota 5-10x faster than direct connections. Galyarder Route now preserves client's cache_control markers when sourceFormat and targetFormat are both Claude, ensuring prompt caching works correctly and dramatically reducing token consumption.
...
- **Kiro Account Ban Documentation** — Documented as upstream AWS anti-fraud false positive, not an Galyarder Route issue (#649)
...
### 🎉 Galyarder Route v3.0.0 — The Free AI Gateway, Now with 67+ Providers
...
Auto-generate and issue Galyarder Route API keys programmatically with per-provider and per-account quota enforcement.
...
```bash
# npm
npm install -g galyarder-route@3.0.0

# Docker
docker pull galyarderlabs/galyarder-route:3.0.0
...
Galyarder Route now automatically refreshes model lists for connected providers every **24 hours**.
...
- **#537 — Gemini CLI OAuth:** Clear actionable error when `GEMINI_OAUTH_CLIENT_SECRET` is missing in Docker/self-hosted deployments. Previously showed cryptic `client_secret is missing` from Google. Now provides specific `docker-compose.yml` and `~/.galyarder-route/.env` instructions.
...
- **#492 — CLI startup:** `galyarder-route` CLI now detects `mise`/`nvm`-managed Node when `app/server.js` is missing and shows targeted fix instructions.
...
- **T04** — `X-Session-Id` header for external sticky routing: `extractExternalSessionId()` in `sessionManager.ts` reads `x-session-id` / `x-galyarder-route-session` headers with `ext:` prefix to avoid collision with internal SHA-256 session IDs. Nginx-compatible (hyphenated header).
...
- **#492** — `galyarder-route` CLI now detects `mise`/`nvm` when `app/server.js` is missing and shows targeted fix
...
- **#485** — Kiro MCP tool calls: closed as external Kiro issue (not Galyarder Route)
...
- **feat(mcp)**: 2 new advanced tool schemas: `galyarder_route_get_provider_metrics` (p50/p95/p99 per provider) and `galyarder_route_explain_route` (routing decision explanation)
- **feat(mcp)**: MCP tool auth scopes updated — `metrics:read` scope added for provider metrics tools
- **feat(mcp)**: `galyarder_route_best_combo_for_task` now accepts `languageHint` parameter for multilingual routing
...
- **fix(startup) #369**: Unify env loading across `npm run start`, `run-standalone.mjs`, and Electron to respect `DATA_DIR/.env → ~/.galyarder-route/.env → ./.env` priority — prevents generating a new `STORAGE_ENCRYPTION_KEY` over an existing encrypted database
...
- **External Pricing Sync (LiteLLM)**: 3-tier pricing resolution (user overrides → synced → defaults). Opt-in via `PRICING_SYNC_ENABLED=true`. MCP tool `galyarder_route_sync_pricing`. 23 new tests.
...
- **GeminiCLI missing projectId (#338)**: Instead of throwing a hard 500 error when `projectId` is missing from stored credentials (e.g. after Docker restart), Galyarder Route now logs a warning and attempts the request — returning a meaningful provider-side error instead of an Galyarder Route crash.
...
- **Intra-Family Model Fallback (T5)**: When a model is unavailable (404/400/403), Galyarder Route now automatically falls back to sibling models from the same family before returning an error (`modelFamilyFallback.ts`).
