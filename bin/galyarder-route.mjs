...
/**
 * GalyarderRoute CLI — Smart AI Router with Auto Fallback
 *
 * Usage:
 *   galyarder-route                          Start the server (default port 20128)
 *   galyarder-route --port 3000              Start on custom port
 *   galyarder-route --no-open                Start without opening browser
 *   galyarder-route --mcp                    Start MCP server (stdio transport for IDEs)
 *   galyarder-route reset-encrypted-columns  Reset broken encrypted credentials
 *   galyarder-route --help                   Show help
 *   galyarder-route --version                Show version
 */
...
function loadEnvFile() {
  ...
  if (home) {
    if (platform() === "win32") {
      ...
      envPaths.push(join(appData, "galyarder-route", ".env"));
    } else {
      envPaths.push(join(home, ".galyarder-route", ".env"));
    }
  }
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
  \x1b[1m\x1b[36m⚡ GalyarderRoute\x1b[0m — Smart AI Router with Auto Fallback

  \x1b[1mUsage:\x1b[0m
    galyarder-route                 Start the server
    galyarder-route --port <port>   Use custom API port (default: 20128)
    galyarder-route --no-open       Don't open browser automatically
    galyarder-route --mcp           Start MCP server (stdio transport for IDEs)
    galyarder-route reset-encrypted-columns  Reset encrypted credentials (recovery)
    galyarder-route --help          Show this help
    galyarder-route --version       Show version

  \x1b[1mMCP Integration:\x1b[0m
    The --mcp flag starts an MCP server over stdio, exposing GalyarderRoute
    tools for AI agents in VS Code, Cursor, Claude Desktop, and Copilot.

    Available tools: galyarder_route_get_health, galyarder_route_list_combos,
    galyarder_route_check_quota, galyarder_route_route_request, and more.

  \x1b[1mConfig:\x1b[0m
    Loads .env from: ~/.galyarder-route/.env or ./.env
    Memory limit: GALYARDER_ROUTE_MEMORY_MB (default: 512)
...
if (args.includes("reset-encrypted-columns")) {
  const dataDir = (() => {
    ...
    if (platform() === "win32") {
      ...
      return join(appData, "galyarder-route");
    }
    const xdg = process.env.XDG_CONFIG_HOME?.trim();
    if (xdg) return join(xdg, "galyarder-route");
    return join(homedir(), ".galyarder-route");
  })();
  ...
  \x1b[1mTo confirm, run:\x1b[0m
    galyarder-route reset-encrypted-columns --force
    `);
    process.exit(0);
  }
...
if (!existsSync(serverJs)) {
  ...
  if (isMise) {
    console.error(
      "  \x1b[33m⚠ mise detected:\x1b[0m If you installed via `npm install -g galyarder-route`,"
    );
    console.error("    try: \x1b[36mnpx galyarder-route@latest\x1b[0m  (downloads a fresh copy)");
    console.error("    or:  \x1b[36mmise exec -- npx galyarder-route\x1b[0m");
  } else if (isNvm) {
    console.error(
      "  \x1b[33m⚠ nvm detected:\x1b[0m Try reinstalling after loading the correct Node version:"
    );
    console.error("    \x1b[36mnvm use --lts && npm install -g galyarder-route\x1b[0m");
  } else {
    console.error("  Try: \x1b[36mnpm install -g galyarder-route\x1b[0m  (reinstall)");
    console.error("  Or:  \x1b[36mnpx galyarder-route@latest\x1b[0m");
  }
  process.exit(1);
}
...
const rawMemory = parseInt(process.env.GALYARDER_ROUTE_MEMORY_MB || "512", 10);
const memoryLimit =
  Number.isFinite(rawMemory) && rawMemory >= 64 && rawMemory <= 16384 ? rawMemory : 512;

const env = {
  ...process.env,
  GALYARDER_ROUTE_PORT: String(port),
...
function shutdown() {
  console.log("\n\x1b[33m⏹ Shutting down GalyarderRoute...\x1b[0m");
  server.kill("SIGTERM");
  setTimeout(() => {
    server.kill("SIGKILL");
    process.exit(0);
...
async function onReady() {
  ...
  console.log(`
  \x1b[32m✔ GalyarderRoute is running!\x1b[0m
