...
> Complete reference for every environment variable recognized by Galyarder Route.
...
Galyarder Route uses **SQLite** (via `better-sqlite3`) for all persistence. These variables control data location, encryption, and lifecycle.

| Variable                         | Default              | Source File                                     | Description                                                                                                        |
| -------------------------------- | -------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `DATA_DIR`                       | `~/.galyarder-route/`      | `src/lib/db/core.ts`                            | Root directory for SQLite DB, backups, and data files. Override for Docker volumes or custom paths.                |
...
| `GALYARDER_ROUTE_CRYPT_KEY`            | _(unset)_            | `src/lib/db/encryption.ts`                      | **Legacy alias** for `STORAGE_ENCRYPTION_KEY`. Accepted as a fallback when the primary variable is absent.         |
| `GALYARDER_ROUTE_API_KEY_BASE64`       | _(unset)_            | `src/lib/db/encryption.ts`                      | **Legacy alias** (Base64-encoded form) accepted as a fallback. Decoded automatically before use.                   |

### Scenarios

| Scenario              | Configuration                                                                    |
| --------------------- | -------------------------------------------------------------------------------- |
| **Local development** | Leave all defaults. DB lives at `~/.galyarder-route/galyarder-route.db`.                     |
...
| **CI/Testing**        | `DATA_DIR=/tmp/galyarder-route-test` — ephemeral, no encryption needed.                |
...
| `GALYARDER_ROUTE_PORT`      | _(unset)_    | `src/lib/runtime/ports.ts` | Takes precedence over `PORT` when running inside Electron or other wrappers.           |
...
| Variable                      | Default               | Source File                              | Description                                                                                               |
| ----------------------------- | --------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `MACHINE_ID_SALT`             | `galyarder-route-salt` | `src/lib/auth`                           | Salt combined with hardware identifiers for machine fingerprinting. Change per-deployment for isolation.  |
...
Galyarder Route provides a two-layer defense: request-side injection scanning and response-side PII stripping.
...
> [!IMPORTANT]
> When deploying behind a reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **must** be set to your public URL (e.g., `https://galyarder-route.example.com`). Without this, OAuth callbacks will fail because the redirect_uri won't match.
...
Controls how Galyarder Route discovers and launches CLI sidecars (Claude Code, Codex, etc.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Allow Galyarder Route to write CLI config files (token refresh, session data).   |
...
```bash
# Mount host binaries into the container and tell Galyarder Route where they are:
...
| Variable                                | Default     | Source File                                 | Description                                                                                                                   |
| --------------------------------------- | ----------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `GALYARDER_ROUTE_BASE_URL`                    | auto-detect | `open-sse/mcp-server/server.ts`             | Explicit URL for MCP/A2A tools to reach Galyarder Route. Overrides localhost auto-detection.                                        |
| `GALYARDER_ROUTE_API_KEY`                     | _(unset)_   | MCP/A2A modules                             | API key for internal MCP tool and A2A skill calls.                                                                            |
| `GALYARDER_ROUTE_API_KEY_ID`                  | _(unset)_   | `open-sse/mcp-server/audit.ts`              | Key ID for MCP audit log attribution.                                                                                         |
| `ROUTER_API_KEY`                        | _(unset)_   | Legacy                                      | Legacy alias for `GALYARDER_ROUTE_API_KEY`.                                                                                         |
| `GALYARDER_ROUTE_MCP_ENFORCE_SCOPES`          | `false`     | `open-sse/mcp-server/server.ts`             | Enforce scope-based access control on MCP tool calls.                                                                         |
| `GALYARDER_ROUTE_MCP_SCOPES`                  | _(all)_     | `open-sse/mcp-server/server.ts`             | Comma-separated scopes: `admin`, `combos`, `health`, `models`, `routing`, `budget`, `metrics`, `pricing`, `memory`, `skills`. |
...
| `GALYARDER_ROUTE_DISABLE_BACKGROUND_SERVICES` | `false`     | `src/instrumentation-node.ts`               | Disable all background services (sync, pricing, model refresh). Useful for CI/test.                                           |
| `GALYARDER_ROUTE_BOOTSTRAPPED`                | `false`     | `src/app/(dashboard)/dashboard/page.tsx`    | Set `true` by bootstrap script after initial setup. Controls setup wizard visibility.                                         |
| `GALYARDER_ROUTE_ALLOW_BODY_PROJECT_OVERRIDE` | `0`         | `open-sse/executors/antigravity.ts`         | Escape hatch: allow request body to override the Antigravity project field.                                                   |

### OAuth CLI Bridge (Internal)

| Variable            | Default     | Source File                     | Description                               |
| ------------------- | ----------- | ------------------------------- | ----------------------------------------- |
| `GALYARDER_ROUTE_SERVER`  | auto-detect | `src/lib/oauth/config/index.ts` | Server URL for CLI↔Galyarder Route auth bridge. |
| `GALYARDER_ROUTE_TOKEN`   | _(unset)_   | `src/lib/oauth/config/index.ts` | Auth token for CLI bridge.                |
| `GALYARDER_ROUTE_USER_ID` | `cli`       | `src/lib/oauth/config/index.ts` | User ID for CLI bridge sessions.          |
| `SERVER_URL`        | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `GALYARDER_ROUTE_SERVER`.      |
| `CLI_TOKEN`         | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `GALYARDER_ROUTE_TOKEN`.       |
| `CLI_USER_ID`       | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `GALYARDER_ROUTE_USER_ID`.     |
...
| `GALYARDER_ROUTE_QODER_WORKSPACE`       | Qoder                   | Alias for `QODER_CLI_WORKSPACE`.                                                  |
...
When enabled, Galyarder Route reorders HTTP headers and JSON body fields to match the exact signature of official CLI tools. This reduces the risk of account flagging while preserving your proxy IP.
...
| Variable                   | Default                         | Description                                                            |
| -------------------------- | ------------------------------- | ---------------------------------------------------------------------- |
| `GALYARDER_ROUTE_MEMORY_MB`      | `256` (Docker) / system default | V8 heap limit. Sets `--max-old-space-size`.                            |
...
| Variable                              | Default | Description                                                                                                   |
| ------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| `GALYARDER_ROUTE_RTK_TRUST_PROJECT_FILTERS` | unset   | Trust project `.rtk/filters.json` without a `.rtk/trust.json` hash. Use only in controlled local development. |

### Low-RAM Docker Example

```bash
GALYARDER_ROUTE_MEMORY_MB=128
...
exclusively. Galyarder Route rewrites requests so those relays accept them. If you only want to use
...
| `NEXT_PUBLIC_GALYARDER_ROUTE_E2E_MODE` | _(unset)_ | E2E test harness                          | Set `true` to enable E2E test mode (relaxed auth, test hooks). |
...
NEXT_PUBLIC_BASE_URL=https://galyarder-route.example.com
BASE_URL=http://localhost:20128
GALYARDER_ROUTE_MEMORY_MB=512
...
NODE_ENV=production
GALYARDER_ROUTE_DISABLE_BACKGROUND_SERVICES=true
...
CORS_ORIGIN=https://galyarder-route.example.com
...
| `INSTANCE_NAME=galyarder-route`                             | Present in old docs/env templates but unused at runtime. May return in a future multi-instance feature. |
