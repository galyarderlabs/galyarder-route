# 🗜️ Prompt Compression Guide — Galyarder Route
...
Galyarder Route implements a modular prompt compression pipeline that runs **proactively** before requests hit upstream providers. This means your token savings happen transparently — no changes needed to your workflow.
...
Galyarder Route documents compression savings from two sources: upstream project benchmarks and
Galyarder Route's own engine composition.
...
For overlapping tool/context payloads, the default Galyarder Route combo stacks the engines:
