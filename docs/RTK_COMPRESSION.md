...
RTK compression is Galyarder Route's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). Galyarder Route uses
...
- `GALYARDER_ROUTE_RTK_TRUST_PROJECT_FILTERS=1` is set.
