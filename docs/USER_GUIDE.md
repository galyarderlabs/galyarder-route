...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying Galyarder Route.
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-galyarder-route-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "galyarder-route/if/glm-4.7" }
    }
  },
  "models": {
    "providers": {
      "galyarder-route": {
        ...
        "apiKey": "your-galyarder-route-api-key",
...
mkdir -p ~/.galyarder-route

# Create .env file (see .env.example)
cp .env.example ~/.galyarder-route/.env

# Start server
galyarder-route
# Or with custom port:
galyarder-route --port 3000
```

The CLI automatically loads `.env` from `~/.galyarder-route/.env` or `./.env`.
...
cd Galyarder Route && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/galyarder-route"
export PORT="20128"
export HOSTNAME="0.0.0.0"
export NODE_ENV="production"
export NEXT_PUBLIC_BASE_URL="http://localhost:20128"
export API_KEY_SECRET="galyarder-route-api-key-secret"

npm run start
# Or: pm2 start npm --name galyarder-route -- start
```
...
pm2 start npm --name galyarder-route -- start

# Or with custom memory limit
GALYARDER_ROUTE_MEMORY_MB=512 pm2 start npm --name galyarder-route -- start
...
docker build -t galyarder-route:cli .

# Portable mode (recommended)
docker run -d --name galyarder-route -p 20128:20128 --env-file ./.env -v galyarder-route-data:/app/data galyarder-route:cli
```
...
Void Linux users can package and install Galyarder Route natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `galyarder-route-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `123456`                             | First login password                                                                                      |
| `DATA_DIR`                              | `~/.galyarder-route`                       | Data directory (db, usage, logs)                                                                          |
| `PORT`                                  | framework default                    | Service port (`20128` in examples)                                                                        |
| `HOSTNAME`                              | framework default                    | Bind host (Docker defaults to `0.0.0.0`)                                                                  |
| `NODE_ENV`                              | runtime default                      | Set `production` for deploy                                                                               |
| `BASE_URL`                              | `http://localhost:20128`             | Server-side internal base URL                                                                             |
| `CLOUD_URL`                             | `https://galyarder-route.dev`              | Cloud sync endpoint base URL                                                                              |
| `API_KEY_SECRET`                        | `galyarder-route-api-key-secret`      | HMAC secret for generated API keys                                                                        |
...
| `GALYARDER_ROUTE_MEMORY_MB`                   | `512`                                | Node.js heap limit in MB                                                                                  |
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-Galyarder-Route-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-Galyarder-Route-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how Galyarder Route translates API requests between providers.
...
Galyarder Route also accepts `x_session_id` and returns the effective session key in `X-Galyarder-Route-Session-Id`.
...
Galyarder Route implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, Galyarder Route can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate Galyarder Route between machines
...
Galyarder Route supports audio transcription via the OpenAI-compatible endpoint:
...
Galyarder Route is available as a native desktop application for Windows, macOS, and Linux.
...
| Variable              | Default | Description                      |
| --------------------- | ------- | -------------------------------- |
| `GALYARDER_ROUTE_PORT`      | `20128` | Server port                      |
| `GALYARDER_ROUTE_MEMORY_MB` | `512`   | Node.js heap limit (64–16384 MB) |
