...
Galyarder Route's combo system currently requires manual configuration: users must know which providers and models are actually working, then manually wire them into combo chains. When providers fail (rate limits, auth errors, model deprecation), combos silently degrade — routing to dead endpoints that timeout or return errors. There is no automated way to:
...
This proposes an **Auto-Assessment Engine** that continuously tests, categorizes, and self-heals combo configurations — making galyarder-route truly "plug and play" for non-technical users.
...
While configuring galyarder-route for production use, we discovered:
...
The assessment state should be visible in the galyarder-route dashboard:
...
# 1. Start galyarder-route with 406 provider connections (51 providers)
