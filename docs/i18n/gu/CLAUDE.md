...
**Galyarder Route** — unified AI proxy/router. One endpoint, 100+ LLM providers, auto-fallback.
...
```
Galyarder Route/              # Root package
├── src/                # Next.js 16 app (TypeScript)
├── open-sse/           # @galyarder-route/open-sse workspace (streaming engine)
...
- **Imports**: external → internal (`@/`, `@galyarder-route/open-sse`) → relative
...
- **Path aliases**: `@/*` → `src/`, `@galyarder-route/open-sse` → `open-sse/`
- **Default port**: 20128 (API + dashboard on same port)
- **Data directory**: `DATA_DIR` env var, defaults to `~/.galyarder-route/`
