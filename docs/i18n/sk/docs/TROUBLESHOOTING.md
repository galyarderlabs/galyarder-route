...
Common problems and solutions for Galyarder Route.
...
| EACCES: permission denied                           | Set `DATA_DIR=/path/to/writable/dir` to override `~/.galyarder-route`                                                                                          |
| Routing strategy not saving                         | Update to v1.4.11+ (Zod schema fix for settings persistence)                                                                                             |
| Login crash / blank page                            | Check Node.js version — see [Node.js Compatibility](#nodejs-compatibility) below                                                                         |
| `dlopen` / `slice is not valid mach-o file` (macOS) | Run `cd $(npm root -g)/galyarder-route/app && npm rebuild better-sqlite3 && galyarder-route` — see [macOS native module rebuild](#macos-native-module-rebuild) below |
...
**Cause:** You are running a Node.js version outside Galyarder Route's approved secure runtime floor. The most common case is running an older Node 20, 22, or 24 patch level that falls below the patched security floor Galyarder Route requires.
...
3. Reinstall Galyarder Route: `npm install -g galyarder-route`
4. Restart: `galyarder-route`
...
```
dlopen(/Users/<user>/.nvm/versions/node/v24.14.1/lib/node_modules/galyarder-route/app/node_modules/better-sqlite3/build/Release/better_sqlite3.node, 0x0001): tried: '...' (slice is not valid mach-o file)
```

**Fix — rebuild for your local environment (no Node.js downgrade required):**

```bash
cd $(npm root -g)/galyarder-route/app
...
**Fix (v3.5.5+):** Galyarder Route now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.
...
Galyarder Route auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://galyarder-route.dev`)
...
When many concurrent requests hit a rate-limited provider, Galyarder Route uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some Galyarder Route users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: Galyarder Route looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in Galyarder Route
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the Galyarder Route logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind Galyarder Route.

---

## Still Stuck?

- **GitHub Issues**: [github.com/galyarderlabs/galyarder-route/issues](https://github.com/galyarderlabs/galyarder-route/issues)
