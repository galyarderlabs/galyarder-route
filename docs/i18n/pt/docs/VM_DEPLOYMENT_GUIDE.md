# Galyarder Route — Deployment Guide on VM with Cloudflare (Português (Portugal))
...
Complete guide to install and configure Galyarder Route on a VM (VPS) with domain managed via Cloudflare.
...
## 2. Install Galyarder Route

### 2.1 Create configuration directory

```bash
mkdir -p /opt/galyarder-route
```

### 2.2 Create environment variables file

```bash
cat > /opt/galyarder-route/.env << ‘EOF’
# === Security ===
...
# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.galyarder-route.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.galyarder-route.online
EOF
```
...
```bash
docker pull galyarderlabs/galyarder-route:latest

docker run -d \
  --name galyarder-route \
  --restart unless-stopped \
  --env-file /opt/galyarder-route/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep galyarder-route
docker logs galyarder-route --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/galyarder-route << ‘NGINX’
...
Keep reverse-proxy stream timeouts aligned with your Galyarder Route timeout env vars. If you raise
...
# Enable Galyarder Route
ln -sf /etc/nginx/sites-available/galyarder-route /etc/nginx/sites-enabled/galyarder-route
...
```bash
docker pull galyarderlabs/galyarder-route:latest
docker stop galyarder-route && docker rm galyarder-route
docker run -d --name galyarder-route --restart unless-stopped \
  --env-file /opt/galyarder-route/.env \
  -p 20128:20128 \
  -v galyarder-route-data:/app/data \
  galyarderlabs/galyarder-route:latest
...
```bash
docker logs -f galyarder-route          # Real-time stream
docker logs galyarder-route --tail 50   # Last 50 lines
```
...
docker cp galyarder-route:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v galyarder-route-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/galyarder-route-data-$(date +%F).tar.gz /data
```
...
docker run --rm -v galyarder-route-data:/data -v $(pwd):/backup \
  alpine sh -c “rm -rf /data/* && tar xzf /backup/galyarder-route-data-YYYY-MM-DD.tar.gz -C /”
docker start galyarder-route
```
...
cd galyarder-routeCloud
npm install
npx wrangler login
npx wrangler deploy
```

See the full documentation at [galyarder-routeCloud/README.md](../galyarder-routeCloud/README.md).
...
| 20128 | Galyarder Route   | Localhost only (via nginx) |
