# Galyarder Route Fly.io 部署指南 (Português (Portugal))
...
本文档记录 Galyarder Route 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `galyarder-route`。
...
- 访问地址：`https://galyarder-route.fly.dev/`
...
- `app = 'galyarder-route'` 决定实际部署到哪个 Fly 应用
...
```toml
app = 'galyarder-route-yourname'
```

注意：

- 控制台里要看的是与 `fly.toml` 里 `app` 一致的应用
- 以前如果用过别的名字，例如 `oroute`，不要和 `galyarder-route` 混淆
...
```powershell
flyctl apps create galyarder-route
```

如果你已经改成别的应用名，把 `galyarder-route` 替换成你的名字。
...
这些参数已经在当前 `galyarder-route` 应用上实际部署：
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://galyarder-route.fly.dev` |
...
```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a galyarder-route
```
...
```powershell
flyctl secrets list -a galyarder-route
```

如果控制台 `Secrets` 页面没有显示你期待的变量，先检查：

- 看的应用是不是 `galyarder-route`
...
```powershell
flyctl secrets set KEY=value -a galyarder-route
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/galyarderlabs/galyarder-route` 的更新，推荐按下面流程执行。
...
```powershell
git remote add upstream https://github.com/galyarderlabs/galyarder-route.git
```
...
5. `flyctl deploy`
6. `flyctl status -a galyarder-route`
7. `flyctl logs --no-tail -a galyarder-route`
...
```powershell
flyctl status -a galyarder-route
```

### 10.2 查看启动日志

```powershell
flyctl logs --no-tail -a galyarder-route
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://galyarder-route.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- 你打开的是另一个应用，例如 `oroute`，不是 `galyarder-route`
...
1. `flyctl auth login`
2. `flyctl apps create galyarder-route`
3. `flyctl secrets set ... -a galyarder-route`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a galyarder-route`
