# Galyarder Route Architecture (Português (Portugal))
...
Galyarder Route is a local AI routing gateway and dashboard built on Next.js.
...
- `/dashboard/endpoint` — route gateway + MCP + A2A + API endpoint tabs
...
flowchart LR
    ...
    subgraph Router[Galyarder Route Local Process]
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/galyarder-route/storage.sqlite` when set, else `~/.galyarder-route/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Galyarder Route Runtime]
        Next[Next.js Server\nPORT=20128]
...
- final client response returned by Galyarder Route; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/galyarder-route` -> `~/.galyarder-route`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@galyarder-route/open-sse` **npm workspace package**. Source code imports it via `@galyarder-route/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t galyarder-route .`
