...
- before the active account is exhausted, Galyarder Route generates a compact structured summary
- after authentication selects a different account for the same session, Galyarder Route injects
...
If the active provider is enabled in `handoffProviders`, Galyarder Route generates a structured
...
Galyarder Route prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, Galyarder Route converts that payload into a `<context_handoff>` system
