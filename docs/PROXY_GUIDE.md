# 🌐 Galyarder Route Proxy Guide

> **Bypass geographic blocks, protect your identity, and route AI traffic through any proxy — with zero configuration complexity.**

Galyarder Route includes a full-featured proxy management system that lets you route upstream AI provider traffic through HTTP, HTTPS, or SOCKS5 proxies. Whether you're in a blocked region, need IP rotation, or want stealth fingerprinting — this guide covers everything.
...
Galyarder Route supports proxy configuration at **four independent scopes**, resolved in priority order:
...
When Galyarder Route sends a request to an upstream provider, it calls `resolveProxyForConnectionFromRegistry()` which checks each level in order:
...
Proxies are included in the **Backup/Restore** system. When you export your Galyarder Route configuration:
...
If you configured proxies in an older version (pre-registry), Galyarder Route automatically migrates them:
...
> 🆕 **Contributed by [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/galyarderlabs/galyarder-route/pull/1847) (Issue [#1788](https://github.com/galyarderlabs/galyarder-route/issues/1788))

Galyarder Route integrates with the **[1proxy](https://1proxy-api.aitradepulse.com)** community platform to provide access to **hundreds of free, validated proxies** from around the world. This is perfect for users who don't have their own proxy infrastructure.
...
1. **Sync** — Galyarder Route fetches validated proxies from the 1proxy API
...
Galyarder Route doesn't just route traffic through a proxy — it makes the traffic look legitimate:
...
For providers that use the CLIProxyAPI pattern, Galyarder Route supports three upstream proxy modes:

| Mode          | Description                                        |
| ------------- | -------------------------------------------------- |
| `native`      | Galyarder Route handles proxy routing directly (default) |
...
This is normal with cheap proxies that drop idle connections. Galyarder Route already handles this by:
...
Make sure the proxy is configured **before** starting the OAuth flow. Galyarder Route routes OAuth token exchange through the configured proxy. Set a global or provider-level proxy first, then connect.
