...
IMAGE_NAME="${1:-galyarder-route-local-hardened}"
CONTAINER_NAME="${2:-galyarder-route-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/galyarder_route_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/galyarder_route_hardening_container_id.txt
