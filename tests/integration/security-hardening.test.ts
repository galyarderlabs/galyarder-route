...
test("package.json does not contain hardcoded legacy secrets", () => {
  ...
  const sensitivePatterns = [
    "galyarder-route-default-secret-change-me",
    "galyarder-route-api-key-secret",
...
test("proxy.ts does not contain hardcoded JWT_SECRET fallback", () => {
  ...
  assert.equal(
    content.includes("galyarder-route-default-secret-change-me"),
    false,
...
test("apiKey.ts does not contain legacy API_KEY_SECRET fallback literal", () => {
  ...
  assert.equal(
    content.includes("galyarder-route-api-key-secret"),
    false,
