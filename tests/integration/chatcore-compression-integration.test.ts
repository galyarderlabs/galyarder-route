...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-compression-"));
...
test("chatCore integration: assigned compression combo applies language packs and output mode", async () => {
  ...
  try {
    ...
    assert.match(firstMessage?.content ?? "", /GalyarderRoute Caveman Output Mode/);
