...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-resilience-http-e2e-"));
...
function createServerProcess(dataDir: string, port: number) {
  ...
  const child = spawn(process.execPath, ["scripts/run-next-playwright.mjs", "dev"], {
    ...
    env: {
      ...
      GALYARDER_ROUTE_DISABLE_TOKEN_HEALTHCHECK: "true",
      GALYARDER_ROUTE_DISABLE_LOCAL_HEALTHCHECK: "true",
      GALYARDER_ROUTE_HIDE_HEALTHCHECK_LOGS: "true",
      GALYARDER_ROUTE_E2E_BOOTSTRAP_MODE: "open",
    },
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 120_000) {
    if (logs.exitInfo) {
      throw new Error(
        [
          `GalyarderRoute exited before it became ready (code=${logs.exitInfo.code}, signal=${logs.exitInfo.signal})`,
          "--- stdout ---",
  ...
  throw new Error(
    [
      `Timed out waiting for GalyarderRoute to start: ${lastError}`,
      "--- stdout ---",
      ...logs.stdoutLines.slice(-40),
      "--- stderr ---",
      ...logs.stderrLines.slice(-40),
