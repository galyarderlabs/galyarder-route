...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-api-auth-"));
...
test("getApiKeyMetadata recognizes GALYARDER_ROUTE_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.GALYARDER_ROUTE_API_KEY = envKey;
