...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/galyarder-route" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/galyarder-route");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/galyarder-route",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/galyarder-route/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "galyarder-route-base",
    ...
    assert.equal(config.repoDir, "/srv/galyarder-route");
    assert.equal(config.composeFile, "/srv/galyarder-route/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "galyarder-route-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart galyarder-route --update-env \|\| true/);
  });
});
