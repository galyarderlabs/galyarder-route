...
test("AntigravityExecutor.buildHeaders includes native headers without GalyarderRoute internals", () => {
  const executor = new AntigravityExecutor();
  ...
  assert.equal(headers["X-Galyarder-Route-Source"], undefined);
});

test("Antigravity header scrub removes GalyarderRoute internal headers", () => {
  const headers = scrubProxyAndFingerprintHeaders({
    Authorization: "Bearer ag-token",
    "X-Galyarder-Route-Source": "galyarder-route",
    "X-Galyarder-Route-No-Cache": "true",
    "X-Forwarded-For": "127.0.0.1",
  });

  assert.equal(headers.Authorization, "Bearer ag-token");
  assert.equal(headers["X-Galyarder-Route-Source"], undefined);
  assert.equal(headers["X-Galyarder-Route-No-Cache"], undefined);
...
test("AntigravityExecutor.transformRequest allows body project overrides when the env flag is enabled", async () => {
  ...
  await withEnv("GALYARDER_ROUTE_ALLOW_BODY_PROJECT_OVERRIDE", "1", async () => {
    const result = await executor.transformRequest(
