...
test("status discrepancy shows both GalyarderRoute and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("GalyarderRoute returned 504"),
    -1,
    "Should indicate GalyarderRoute returned its own status"
  );
});
    ...
    pipelinePayloads: {
      streamChunks: {
        provider: [
          ...
          ": x-galyarder-route-latency-ms=1\n",
          "data: [DONE]\n\n",
        ],
      },
    },
