...
const DUMMY_HOME = path.join(os.tmpdir(), "galyarder-route-qwen-key-test-" + Date.now());
...
test.afterEach(async () => {
  ...
  if (process.env.DATA_DIR?.includes("galyarder-route-qwen-key-test")) {
...
test("getOrCreateApiKey() creates DB-backed key when no keyId provided", async () => {
  ...
  // Key should NOT be the placeholder "sk_galyarder_route"
  assert.notEqual(apiKey, "sk_galyarder_route", "Should not return placeholder");
...
test("Qwen guide-settings POST creates valid DB-backed key (no keyId)", async () => {
  ...
  assert.notEqual(apiKey, "sk_galyarder_route", "Should not use placeholder");
  assert.ok(apiKey.startsWith("sk-"), "Key should start with sk- prefix");
