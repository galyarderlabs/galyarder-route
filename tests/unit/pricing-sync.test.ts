...
import { transformToGalyarderRoute } from "../../src/lib/pricingSync.ts";

// ─── transformToGalyarderRoute ────────────────────────────────

describe("transformToGalyarderRoute", () => {
  test("converts LiteLLM per-token pricing to GalyarderRoute per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToGalyarderRoute(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToGalyarderRoute(raw);
