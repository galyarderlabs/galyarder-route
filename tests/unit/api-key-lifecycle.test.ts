...
test.after(() => {
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_GALYARDER_ROUTE_API_KEY === undefined) delete process.env.GALYARDER_ROUTE_API_KEY;
  else process.env.GALYARDER_ROUTE_API_KEY = ORIGINAL_GALYARDER_ROUTE_API_KEY;
...
test("validateApiKey accepts configured environment API keys", async () => {
  process.env.GALYARDER_ROUTE_API_KEY = "sk-env-lifecycle-test";
