...
const DUMMY_HOME = path.join(os.tmpdir(), "galyarder-route-qwen-test-" + Date.now());
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider["galyarder-route"].npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider["galyarder-route"].options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider["galyarder-route"].options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider["galyarder-route"].models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.galyarder-route", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider["galyarder-route"].options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider["galyarder-route"].options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider["galyarder-route"].models, {
