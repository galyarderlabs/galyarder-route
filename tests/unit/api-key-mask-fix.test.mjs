...
async function resolveApiKey(apiKeyId, apiKey) {
  ...
  return apiKey || "sk_galyarder_route";
}
...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_galyarder_route when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_galyarder_route");
  });

  it("falls back to sk_galyarder_route when both are undefined", async () => {
    mockKeyStore.clear();
