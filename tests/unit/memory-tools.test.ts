...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-memory-tools-"));
...
test("memory add stores entries with default session and metadata", async () => {
  const result = await memoryTools.galyarder_route_memory_add.handler({
    apiKeyId: "key-add",
    type: "factual",
    key: "pref:language",
...
test("memory clear deletes only older filtered entries and reports the deleted count", async () => {
  ...
  const result = await memoryTools.galyarder_route_memory_clear.handler({
    apiKeyId: "key-clear",
    type: "factual",
