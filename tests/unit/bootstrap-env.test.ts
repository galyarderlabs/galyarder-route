...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".galyarder-route") });
  } finally {
...
test("bootstrapEnv prefers ~/.galyarder-route/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
