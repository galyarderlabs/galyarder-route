...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-providers-validate-route-"));
process.env.DATA_DIR = TEST_DATA_DIR;
const originalAllowPrivateProviderUrls = process.env.GALYARDER_ROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
...
test.after(() => {
  ...
  if (originalAllowPrivateProviderUrls === undefined) {
    delete process.env.GALYARDER_ROUTE_ALLOW_PRIVATE_PROVIDER_URLS;
  } else {
    process.env.GALYARDER_ROUTE_ALLOW_PRIVATE_PROVIDER_URLS = originalAllowPrivateProviderUrls;
  }
});
...
test("providers validate route allows private baseUrl values when opt-in env is enabled", async () => {
  await resetStorage();
  process.env.GALYARDER_ROUTE_ALLOW_PRIVATE_PROVIDER_URLS = "true";
