...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-chatcore-translation-"));
...
test("chatCore serves a cached idempotent response without hitting the provider twice", async () => {
  ...
  assert.equal(second.result.response.headers.get("X-Galyarder-Route-Idempotent"), "true");
...
test("chatCore returns a semantic cache HIT for repeated deterministic requests", async () => {
  ...
  assert.equal(first.calls.length, 1);
  assert.equal(first.result.response.headers.get("X-Galyarder-Route-Cache"), "MISS");
  assert.equal(second.calls.length, 0);
  assert.equal(second.result.response.headers.get("X-Galyarder-Route-Cache"), "HIT");
...
test("chatCore skips semantic cache when disabled in settings", async () => {
  ...
  assert.equal(first.result.response.headers.get("X-Galyarder-Route-Cache"), "MISS");
  assert.equal(second.result.response.headers.get("X-Galyarder-Route-Cache"), "MISS");

  const payload = (await second.result.response.json()) as any;
  assert.equal(payload.choices[0].message.content, "fresh-2");
});

test("chatCore attaches GalyarderRoute response metadata headers to non-stream responses", async () => {
  const { result } = await invokeChatCore({
    provider: "claude",
    model: "claude-sonnet-4-6",
    body: {
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-Galyarder-Route-Provider"), "cc");
  assert.equal(result.response.headers.get("X-Galyarder-Route-Model"), "claude-sonnet-4-6");
  assert.equal(result.response.headers.get("X-Galyarder-Route-Cache-Hit"), "false");
  assert.equal(result.response.headers.get("X-Galyarder-Route-Tokens-In"), "12");
  assert.equal(result.response.headers.get("X-Galyarder-Route-Tokens-Out"), "3");
  assert.ok(Number(result.response.headers.get("X-Galyarder-Route-Latency-Ms")) >= 0);
  assert.match(String(result.response.headers.get("X-Galyarder-Route-Response-Cost")), /^\d+\.\d{10}$/);
});
...
test("chatCore injects fallback user for Qwen OAuth requests without user", async () => {
  ...
  assert.equal(result.success, true);
  assert.equal(call.body.user, "galyarder-route-qwen-oauth");
});
...
test("chatCore records Claude prompt cache and cache usage metadata in call logs", async () => {
  ...
  assert.equal(result.success, true);
  assert.ok(detail);
  assert.equal(detail.requestBody._galyarderRoute.claudePromptCache.applied, true);
  assert.equal(detail.requestBody._galyarderRoute.claudePromptCache.totalBreakpoints, 4);
  assert.equal(detail.responseBody._galyarderRoute.claudePromptCache.applied, true);
  assert.equal(detail.responseBody._galyarderRoute.claudePromptCache.totalBreakpoints, 4);
  assert.equal(typeof detail.responseBody._galyarderRoute.claudePromptCache.anthropicBeta, "string");
  assert.match(detail.responseBody._galyarderRoute.claudePromptCache.anthropicBeta, /prompt-caching/i);
  assert.deepEqual(detail.responseBody._galyarderRoute.claudePromptCacheUsage, {
    cacheReadTokens: 4,
...
test("chatCore injects progress events into streaming responses when requested", async () => {
  const { result } = await invokeChatCore({
    ...
    model: "gpt-4o-mini",
    accept: "text/event-stream",
    requestHeaders: { "x-galyarder-route-progress": "true" },
    body: {
      model: "gpt-4o-mini",
      stream: true,
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-Galyarder-Route-Progress"), "enabled");
...
test("chatCore emits final SSE metadata comments before [DONE] on streaming responses", async () => {
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-Galyarder-Route-Provider"), "openai");
  assert.equal(result.response.headers.get("X-Galyarder-Route-Model"), "gpt-4o-mini");
  assert.match(streamText, /: x-galyarder-route-response-cost=\d+\.\d{10}/);
  assert.match(streamText, /: x-galyarder-route-tokens-in=\d+/);
  assert.match(streamText, /: x-galyarder-route-tokens-out=\d+/);
  assert.ok(
    streamText.indexOf(": x-galyarder-route-response-cost=") < streamText.indexOf("data: [DONE]")
  );
});
...
test("chatCore skips streaming cache when X-Galyarder-Route-No-Cache header is set", async () => {
  let upstreamHits = 0;
  const sharedBody = {
    model: "gpt-4o-mini",
    stream: true,
  ...
  const first = await invokeChatCore({
    ...
    model: "gpt-4o-mini",
    accept: "text/event-stream",
    requestHeaders: { "x-galyarder-route-no-cache": "true" },
    body: sharedBody,
    responseFormat: "openai",
    responseFactory() {
      upstreamHits += 1;
