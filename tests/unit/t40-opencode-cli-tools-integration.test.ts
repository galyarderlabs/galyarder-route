...
test("T40: OpenCode config generator includes endpoint and selected API key", () => {
  ...
  assert.equal(mergedConfig.provider["galyarder-route"].options.baseURL, "http://localhost:20128/v1");
  assert.equal(mergedConfig.provider["galyarder-route"].options.apiKey, "sk_test_opencode");
});
...
test("T40: OpenCode config document uses current provider schema", () => {
  ...
  assert.ok(configDocument.provider["galyarder-route"]);
  assert.equal(configDocument.provider["galyarder-route"].npm, "@ai-sdk/openai-compatible");
  assert.equal(configDocument.provider["galyarder-route"].options.baseURL, "http://localhost:20128/v1");
  assert.equal(configDocument.provider["galyarder-route"].options.apiKey, "sk_test_opencode");
  assert.deepEqual(Object.keys(configDocument.provider["galyarder-route"].models), [
    "cc/claude-sonnet-4-20250514",
    "gg/gemini-2.5-pro",
  ]);
  assert.equal(
    configDocument.provider["galyarder-route"].models["cc/claude-sonnet-4-20250514"].name,
    "Claude Sonnet 4.5"
  );
  assert.equal(
    configDocument.provider["galyarder-route"].models["gg/gemini-2.5-pro"].name,
    "Gemini 2.5 Pro"
  );
...
test("T40: OpenCode merge preserves unrelated config and updates only provider.galyarder-route", () => {
  const mergedConfig = mergeOpenCodeConfig(
    {
  ...
  assert.deepEqual(mergedConfig.provider["galyarder-route"].models, {
