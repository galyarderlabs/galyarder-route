...
test("wantsProgress detects X-Galyarder-Route-Progress header correctly", () => {
  const headersObj = { "x-galyarder-route-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-galyarder-route-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-galyarder-route-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-galyarder-route-progress": "false" }), false);
