...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-a2a-enabled-route-"));
const ORIGINAL_DATA_DIR = process.env.DATA_DIR;
const ORIGINAL_GALYARDER_ROUTE_API_KEY = process.env.GALYARDER_ROUTE_API_KEY;

process.env.DATA_DIR = TEST_DATA_DIR;
delete process.env.GALYARDER_ROUTE_API_KEY;
...
test.after(() => {
  ...
  if (ORIGINAL_GALYARDER_ROUTE_API_KEY === undefined) {
    delete process.env.GALYARDER_ROUTE_API_KEY;
  } else {
    process.env.GALYARDER_ROUTE_API_KEY = ORIGINAL_GALYARDER_ROUTE_API_KEY;
  }
});
...
test("A2A JSON-RPC checks auth before returning disabled state", async () => {
  process.env.GALYARDER_ROUTE_API_KEY = "test-secret";
