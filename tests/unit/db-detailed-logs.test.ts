...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-db-detailed-logs-"));
...
test("saveRequestDetailLog persists protected payloads and compacted stream summaries", () => {
  ...
  assert.deepEqual((row as any).provider_response._galyarder_route_stream, {
    format: "sse-json",
