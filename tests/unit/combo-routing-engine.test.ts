...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-combo-routing-"));
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        ...
        "x-galyarder-route-provider-breaker": "open",
      },
    }
  );
}
...
test("handleComboChat context cache protection sanitizes streamed text tags from client output", async () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(result.headers.get("X-Galyarder-Route-Model"), "openai/gpt-4o-mini");
