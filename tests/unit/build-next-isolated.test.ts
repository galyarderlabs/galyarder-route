...
async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-route-build-next-isolated-"));
...
test("getTransientBuildPaths only moves _tasks when explicitly enabled", () => {
  const paths = getTransientBuildPaths("/repo", { GALYARDER_ROUTE_BUILD_MOVE_TASKS: "1" });
