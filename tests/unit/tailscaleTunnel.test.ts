...
const TEST_DATA_DIR = await fs.mkdtemp(path.join(os.tmpdir(), "galyarder-route-tailscale-"));
...
test("getTailscaleUrlFromStatusPayload normalizes DNS names into HTTPS URLs", () => {
  assert.equal(
    tailscaleTunnel.getTailscaleUrlFromStatusPayload({
      Self: { DNSName: "galyarder-route-demo.tail123.ts.net." },
    }),
    "https://galyarder-route-demo.tail123.ts.net"
  );
});
...
test("getTailscaleTunnelStatus reflects a logged-in running funnel", async () => {
  ...
  assert.equal(status.tunnelUrl, "https://galyarder-route-demo.tail123.ts.net");
  assert.equal(status.apiUrl, "https://galyarder-route-demo.tail123.ts.net/v1");
});
...
test("enableTailscaleTunnel stores the funnel URL and disableTailscaleTunnel clears it", async () => {
  ...
  process.env.TAILSCALE_TEST_FUNNEL_OUTPUT = "Available at https://galyarder-route-demo.tail123.ts.net";
  ...
  if (enabled.success) {
    assert.equal(enabled.tunnelUrl, "https://galyarder-route-demo.tail123.ts.net");
  }
  assert.equal(settingsAfterEnable.tailscaleEnabled, true);
  assert.equal(settingsAfterEnable.tailscaleUrl, "https://galyarder-route-demo.tail123.ts.net");
