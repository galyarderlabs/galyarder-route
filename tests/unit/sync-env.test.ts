...
function createTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-sync-env-"));
}
...
test("syncEnv creates .env from .env.example and generates blank secrets", () => {
  ...
  // find the user's real DB at ~/.galyarder-route/ during tests
  ...
  try {
    ...
    assert.match(envContent, /^MACHINE_ID_SALT=galyarder-route-/m);
