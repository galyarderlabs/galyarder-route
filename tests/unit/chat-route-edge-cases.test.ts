...
test("Test 3: handleChat returns cached response directly for Semantic Cache hits", async () => {
  ...
  assert.equal(res2.headers.get("X-Galyarder-Route-Cache"), "HIT");
});

test("Test 4: handleChat supports X-Galyarder-Route-Progress tracking header for streams", async () => {
  ...
  const response = await handleChat(
    buildRequest({
      headers: {
        "X-Galyarder-Route-Progress": "true",
      },
      body: {
        model: "openai/gpt-4",
        stream: true,
  ...
  assert.equal(response.headers.get("X-Galyarder-Route-Progress"), "enabled");
...
test("handleChat returns cached response directly for Idempotency hits", async () => {
  ...
  assert.equal(response2.headers.get("X-Galyarder-Route-Idempotent"), "true");
...
test("handleChat returns Semantic Cache hit", async () => {
  ...
  const json2 = (await response2.json()) as any;
  assert.equal(response2.status, 200);
  assert.equal(response2.headers.get("X-Galyarder-Route-Cache"), "HIT");
