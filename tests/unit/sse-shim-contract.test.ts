...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "galyarder-route-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@galyarder-route\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@galyarder-route\/open-sse\/services\/model\.ts/);
