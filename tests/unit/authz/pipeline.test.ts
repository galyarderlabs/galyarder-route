...
function resetEnvironment() {
  ...
  globalThis.__galyarderRouteShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
}
...
test("runAuthzPipeline redirects unauthenticated dashboard pages to login", async () => {
  ...
  assert.equal(response.headers.get("x-galyarder-route-route-class"), "MANAGEMENT");
...
test("runAuthzPipeline rejects oversized API bodies before auth", async () => {
  ...
  assert.equal(response.headers.get("x-galyarder-route-route-class"), "CLIENT_API");
  assert.ok(response.headers.get("x-request-id"));
  assert.equal(
...
test("runAuthzPipeline rejects new API requests during shutdown drain", async () => {
  globalThis.__galyarderRouteShutdown = { init: true, shuttingDown: true, activeRequests: 0 };
