...
test("auto update config normalizes env values and local source installs to source mode", () => {
  const config = autoUpdate.getAutoUpdateConfig({
    DATA_DIR: "/tmp/galyarder-route-data",
    AUTO_UPDATE_MODE: "npm",
    AUTO_UPDATE_REPO_DIR: "/workspace/custom-galyarder-route",
    AUTO_UPDATE_COMPOSE_FILE: "/workspace/custom-galyarder-route/compose.yml",
    AUTO_UPDATE_COMPOSE_PROFILE: "desktop",
    AUTO_UPDATE_SERVICE: "galyarder-route-desktop",
    AUTO_UPDATE_GIT_REMOTE: "upstream",
    AUTO_UPDATE_PATCH_COMMITS: "abc123, def456 ghi789",
    AUTO_UPDATE_LOG_PATH: "/tmp/galyarder-route-data/auto-update.log",
  });

  assert.equal(config.mode, "source");
  assert.equal(config.repoDir, "/workspace/custom-galyarder-route");
  assert.equal(config.composeFile, "/workspace/custom-galyarder-route/compose.yml");
  assert.equal(config.composeProfile, "desktop");
  assert.equal(config.composeService, "galyarder-route-desktop");
  assert.equal(config.gitRemote, "upstream");
  assert.deepEqual(config.patchCommits, ["abc123", "def456", "ghi789"]);
  assert.equal(config.logPath, "/tmp/galyarder-route-data/auto-update.log");
});
  ...
  const sourceValidation = await autoUpdate.validateAutoUpdateRuntime(
    {
      ...
      composeProfile: "cli",
      composeService: "galyarder-route-cli",
      gitRemote: "origin",
      patchCommits: [],
      logPath: "/tmp/log",
    },
...
test("auto update script builders generate npm, source, and docker-compose scripts with quoting and patch commits", () => {
  ...
  assert.match(npmScript, /npm install -g galyarder-route@3.6.0/);
  assert.match(npmScript, /pm2 restart galyarder-route \|\| true/);
...
test("launchAutoUpdate returns validation failures and starts detached update scripts when runtime is supported", async () => {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-autoupdate-"));
  ...
  const started = await autoUpdate.launchAutoUpdate({
    ...
    env: {
      ...
      AUTO_UPDATE_SERVICE: "galyarder-route-cli",
