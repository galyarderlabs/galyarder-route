...
test("runMigrations applies pending files sequentially in version order", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version FROM _galyarder_route_migrations ORDER BY version").all(),
...
test("runMigrations skips versions that are already tracked as applied", serial, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        db
          .prepare("SELECT COUNT(*) AS count FROM _galyarder_route_migrations WHERE version = ?")
          .get("001") as any
      ).count,
      1
    );
...
test(
  ...
  async () => {
    ...
    `);
      ...
      assert.deepEqual(
        db.prepare("SELECT version, name FROM _galyarder_route_migrations WHERE version = ?").get("032"),
...
test("invalid file names are ignored while valid migrations still run", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version, name FROM _galyarder_route_migrations ORDER BY version").all(),
