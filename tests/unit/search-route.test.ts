...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-search-route-"));
...
test("v1 search POST uses stored Linkup credentials and returns normalized results", async () => {
  ...
  try {
    ...
    assert.equal(body.query, "galyarder-route linkup");
