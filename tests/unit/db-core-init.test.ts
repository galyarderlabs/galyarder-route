...
function cleanupGlobalDb() {
  try {
    if (globalThis.__galyarderRouteDb?.open) {
      globalThis.__galyarderRouteDb.close();
    }
  } catch {}

  delete globalThis.__galyarderRouteDb;
}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("galyarder-route-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _galyarder_route_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("galyarder-route-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("galyarder-route-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "galyarder-route")
              : path.join(fakeHome, ".galyarder-route");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("galyarder-route-db-build-");
...
test("getDbInstance surfaces invalid DATA_DIR paths as sqlite open failures", serial, async () => {
  const sandboxDir = makeTempDir("galyarder-route-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-legacy-empty-");
    ...
    try {
      await withEnv({ DATA_DIR: dataDir }, async () => {
        ...
        assert.ok(
          db
            .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?")
            .get("_galyarder_route_migrations")
        );
        assert.equal(
          db
            .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?")
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      CREATE TABLE _galyarder_route_migrations (
        version TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        applied_at TEXT NOT NULL DEFAULT (datetime('now'))
      );

      INSERT INTO _galyarder_route_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _galyarder_route_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("galyarder-route-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
