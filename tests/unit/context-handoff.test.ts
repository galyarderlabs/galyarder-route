...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._galyarderRouteSkipContextRelay, true);
  assert.equal(calls[0].body._galyarderRouteInternalRequest, "context-handoff");
});
