...
test("CodexExecutor.transformRequest preserves store-enabled responses state when explicitly enabled", () => {
  ...
  const body = {
    _nativeCodexPassthrough: true,
    _galyarderRouteResponsesStore: true,
    instructions: "keep this",
  ...
  assert.equal(result._galyarderRouteResponsesStore, undefined);
...
test("Codex internal websocket bridge rejects non-object JSON payloads", async () => {
  await withEnv({ GALYARDER_ROUTE_WS_BRIDGE_SECRET: "bridge-secret" }, async () => {
    const { POST } = await import("../../src/app/api/internal/codex-responses-ws/route.ts");

    const response = await POST(
      new Request("http://galyarder-route.local/api/internal/codex-responses-ws", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-galyarder-route-ws-bridge-secret": "bridge-secret",
        },
