...
test("Gemini OAuth config reads env while registry ships without Google OAuth secrets", () => {
  assert.equal(
    GEMINI_CONFIG.clientSecret,
    process.env.GEMINI_CLI_OAUTH_CLIENT_SECRET || process.env.GEMINI_OAUTH_CLIENT_SECRET || ""
  );
  assert.equal(REGISTRY.gemini.oauth.clientIdDefault, "");
  assert.equal(REGISTRY.gemini.oauth.clientSecretDefault, "");
  assert.equal(REGISTRY["gemini-cli"].oauth.clientIdDefault, "");
  assert.equal(REGISTRY["gemini-cli"].oauth.clientSecretDefault, "");
  assert.equal(REGISTRY.antigravity.oauth.clientIdDefault, "");
  assert.equal(REGISTRY.antigravity.oauth.clientSecretDefault, "");
});
