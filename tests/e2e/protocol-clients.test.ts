...
const BASE_URL = process.env.GALYARDER_ROUTE_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.GALYARDER_ROUTE_API_KEY || "";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          GALYARDER_ROUTE_BASE_URL: BASE_URL,
          GALYARDER_ROUTE_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("galyarder_route_get_health");
        expect(toolNames).toContain("galyarder_route_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=galyarder_route_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "galyarder_route_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
