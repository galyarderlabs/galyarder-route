/**
 * GalyarderRoute Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead galyarder-route:// protocol (no handler existed)
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "galyarder-route");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "galyarder-route");

  return path.join(require("os").homedir(), ".galyarder-route");
}
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    if (Notification.isSupported()) {
      const notification = new Notification({
        title: "GalyarderRoute Update Ready",
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.galyarder-route.online https://*.galyarder-route.dev`,
...
function createTray() {
  ...
  tray.setToolTip("GalyarderRoute");
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.GALYARDER_ROUTE_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by GalyarderRoute bootstrap",
        "",
