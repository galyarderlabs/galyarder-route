# galyarder-route — Agent Guidelines
...
- **Database**: better-sqlite3 (SQLite) — `DATA_DIR` configurable, default `~/.galyarder-route/`
...
- Path aliases: `@/*` → `src/`, `@galyarder-route/open-sse` → `open-sse/`, `@galyarder-route/open-sse/*` → `open-sse/*`
...
- **Order**: external → internal (`@/`, `@galyarder-route/open-sse`) → relative (`./`, `../`)
...
- **`migrationRunner.ts`**: Applies versioned SQL files from `db/migrations/` inside transactions.
  Tracks applied migrations in `_galyarder_route_migrations` table.
...
- **Transports**: stdio (CLI `galyarder-route --mcp`), SSE (`/api/mcp/sse`), Streamable HTTP
...
- **Built-in skills**: Ship with Galyarder Route (e.g., quota management, routing). Located
