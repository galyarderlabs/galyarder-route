...
import { buildGalyarderRouteResponseMetaHeaders } from "@/domain/galyarderRouteResponseMeta";
...
function cloneBoundedChatLogPayload(value: unknown, depth = 0): unknown {
  ...
  if (Array.isArray(value)) {
    ...
    if (value.length > CHAT_LOG_ARRAY_TAIL_ITEMS) {
      return [
        {
          _galyarder_route_truncated_array: true,
  ...
  if (entries.length > CHAT_LOG_MAX_OBJECT_KEYS) {
    result._galyarder_route_truncated_keys = entries.length - CHAT_LOG_MAX_OBJECT_KEYS;
  }
  return result;
}
...
function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _galyarderRoute: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._galyarderRoute &&
    typeof payload._galyarderRoute === "object" &&
    !Array.isArray(payload._galyarderRoute)
      ? payload._galyarderRoute
      : {};
  return {
    ...payload,
...
export async function handleChatCore({
    ...
    return {
      ...
      response: new Response(JSON.stringify(cachedIdemp.response), {
        ...
        headers: {
          ...
          "X-Galyarder-Route-Idempotent": "true",
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to GalyarderRoute fallback for ${provider}`
    );
  }
  ...
  const pipelineSessionId =
    (clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function"
      ? clientRawRequest.headers.get("x-galyarder-route-session-id")
      : getHeaderValueCaseInsensitive(
          clientRawRequest?.headers ?? null,
          "x-galyarder-route-session-id"
        )) || skillRequestId;
  ...
  // They are accepted for compatibility at the GalyarderRoute API boundary only.
  if (body && typeof body === "object") {
    ...
    if (cached) {
      ...
      return {
        ...
        response: new Response(JSON.stringify(cached), {
          headers: {
            "Content-Type": "application/json",
            [GALYARDER_ROUTE_RESPONSE_HEADERS.cache]: "HIT",
            ...buildGalyarderRouteResponseMetaHeaders({
              provider,
              model,
              cacheHit: true,
  ...
  const executeProviderRequest = async (modelToCall = effectiveModel, allowDedup = false) => {
    const execute = async () => {
      ...
      if (isQwenOAuthRequest && !hasValidQwenUser) {
        bodyToSend = { ...bodyToSend, user: "galyarder-route-qwen-oauth" };
  ...
  if (!stream) {
    ...
    return {
      ...
      response: new Response(JSON.stringify(translatedResponse), {
        headers: {
          "Content-Type": "application/json",
          [GALYARDER_ROUTE_RESPONSE_HEADERS.cache]: "MISS",
          ...buildGalyarderRouteResponseMetaHeaders({
            provider,
            model,
            cacheHit: false,
  ...
  if (progressEnabled) {
    ...
    responseHeaders[GALYARDER_ROUTE_RESPONSE_HEADERS.progress] = "enabled";
  } else {
