...
const RESPONSES_STORE_MARKER = "_galyarderRouteResponsesStore";
...
export function openaiResponsesToOpenAIRequest(
    ...
    for (const toolValue of tools) {
      ...
      if (
        ...
        throw unsupportedFeature(
          `Unsupported Responses API feature: ${toolType} tool type is not supported by galyarder-route`
        );
      }
    }
  }
  ...
  if (root.background) {
    throw unsupportedFeature(
      "Unsupported Responses API feature: background mode is not supported by galyarder-route"
    );
  }
  ...
  if (
    ...
    if (tcType === "function" && tc.name !== undefined && !tc.function) {
      ...
      throw unsupportedFeature(
        `Unsupported Responses API feature: tool_choice type '${tcType}' is not supported by galyarder-route`
      );
    }
  }
