...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._galyarderRouteResponsesStore;
  delete body._galyarderRouteResponsesStore;
  return marker;
}

export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  // GalyarderRoute is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // These are often injected by GalyarderRoute's fallback logic for store=true,
