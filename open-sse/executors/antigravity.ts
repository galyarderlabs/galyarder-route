...
export class AntigravityExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials): AntigravityRequestEnvelope | Response {
    ...
    // Opt-in escape hatch: set GALYARDER_ROUTE_ALLOW_BODY_PROJECT_OVERRIDE=1.
    const projectId =
    ...
    if (!projectId) {
      ...
      const errorMsg =
        "Missing Google projectId for Antigravity account. Please reconnect OAuth in Providers → Antigravity so GalyarderRoute can fetch your Cloud Code project.";
      const errorBody = {
        error: {
          message: errorMsg,
