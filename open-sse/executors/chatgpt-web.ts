...
// GalyarderRoute model ID → ChatGPT internal slug. GalyarderRoute uses dot-form IDs
 ...
 *   1. GalyarderRoute dot-form id (`gpt-5.4-thinking-mini`) — every thinking
 ...
 * dozen "GalyarderRoute" entries clutter the sidebar and can interact with
 ...
 * often reach GalyarderRoute from a container while the user's browser needs a
...
function derivePublicBaseUrl(
  ...
  const explicitPublicBase = normalizePublicBaseUrl(process.env.GALYARDER_ROUTE_PUBLIC_BASE_URL);
  if (explicitPublicBase) {
    log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: using GALYARDER_ROUTE_PUBLIC_BASE_URL`);
    return explicitPublicBase;
  }

  const headerBase = deriveHeaderBaseUrl(clientHeaders);
  const configuredBase =
    normalizePublicBaseUrl(process.env.GALYARDER_ROUTE_BASE_URL) ||
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from GalyarderRoute's
 ...
 * and return a browser-fetchable GalyarderRoute URL.
 */
...
function configuredAsyncImageTimeoutMs(): number {
  const raw = Number(process.env.GALYARDER_ROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS);
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // GalyarderRoute's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an GalyarderRoute
