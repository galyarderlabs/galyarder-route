...
export class GeminiCLIExecutor extends BaseExecutor {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `[GalyarderRoute] onboardUser returned ${response.status} on attempt ${attempt + 1}`
          );
        }
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        console.warn(`[GalyarderRoute] onboardUser attempt ${attempt + 1} failed (${msg})`);
      }
   ...
   * Native Gemini CLI refreshes this every 30 seconds — GalyarderRoute stores it once
  ...
  async _doRefresh(accessToken: string, model = "unknown"): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[GalyarderRoute] loadCodeAssist returned ${response.status} — falling back to stored projectId`
        );
        return null;
      }
      ...
      if (!projectId) {
        console.warn(
          "[GalyarderRoute] managed project onboarding failed — falling back to stored projectId"
        );
        return null;
      }
