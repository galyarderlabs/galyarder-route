/**
 * GalyarderRoute MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. galyarder_route_compression_status   — Get compression config, analytics, and cache stats
 *   2. galyarder_route_compression_configure — Update compression settings
 */
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("galyarder_route_compression_status", args, result, duration, true);
    ...
    await logToolCall(
      "galyarder_route_compression_status",
      args,
      { error: errorMessage },
      duration,
      false,
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("galyarder_route_compression_configure", args, result, duration, true);
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    await logToolCall(
      "galyarder_route_compression_configure",
      args,
      { error: errorMessage },
      duration,
      false,
...
export const compressionTools = {
  ...
  galyarder_route_set_compression_engine: {
  ...
  galyarder_route_list_compression_combos: {
  ...
  galyarder_route_compression_combo_stats: {
