...
declare global {
  var __galyarderRouteMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__galyarderRouteMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__galyarderRouteMcpAuditDb = database;
}
 ...
 * Uses the same SQLite database as the main GalyarderRoute app.
 */
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".galyarder-route", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.GALYARDER_ROUTE_API_KEY_ID || null;
