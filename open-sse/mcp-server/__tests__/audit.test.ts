...
describe("MCP audit shutdown", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    globalThis.__galyarderRouteMcpAuditDb = undefined;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "galyarder-route-mcp-audit-"));
  ...
  it("checkpoints and closes the audit database during shutdown", async () => {
    ...
    await audit.logToolCall("galyarder_route_get_health", { ok: true }, { ok: true }, 12, true);
  ...
  it("still closes the audit database when checkpoint fails", async () => {
    ...
    await audit.logToolCall("galyarder_route_get_health", {}, {}, 5, true);
    expect(audit.closeAuditDb()).toBe(true);
    expect(mockDb.close).toHaveBeenCalledTimes(1);
  });
});
