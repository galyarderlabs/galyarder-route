/**
 * GalyarderRoute MCP Server — Model Context Protocol server exposing
 * GalyarderRoute gateway intelligence as tools for AI agents.
 *
 * Supports two transports:
 *   1. stdio  — for IDE integration (VS Code, Cursor, Claude Desktop)
 *   2. HTTP   — for remote/programmatic access
 *
 * Tools wrap existing GalyarderRoute API endpoints and add intelligence
...
import { resolveGalyarderRouteBaseUrl } from "../../src/shared/utils/resolveGalyarderRouteBaseUrl.ts";

// ============ Configuration ============

const GALYARDER_ROUTE_BASE_URL = resolveGalyarderRouteBaseUrl();
const GALYARDER_ROUTE_API_KEY = process.env.GALYARDER_ROUTE_API_KEY || "";
const MCP_ENFORCE_SCOPES = process.env.GALYARDER_ROUTE_MCP_ENFORCE_SCOPES === "true";
const MCP_ALLOWED_SCOPES = new Set(
  (process.env.GALYARDER_ROUTE_MCP_SCOPES || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
...
/**
 * Internal fetch helper that calls GalyarderRoute API endpoints.
 */
async function omniRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  const url = `${GALYARDER_ROUTE_BASE_URL}${path}`;
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
    ...(GALYARDER_ROUTE_API_KEY ? { Authorization: `Bearer ${GALYARDER_ROUTE_API_KEY}` } : {}),
  ...
  if (!response.ok) {
    ...
    throw new Error(`GalyarderRoute API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    ...
    await logToolCall("galyarder_route_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  ...
  try {
    ...
    await logToolCall("galyarder_route_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  ...
  try {
    ...
    await logToolCall("galyarder_route_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  ...
  try {
    ...
    await logToolCall("galyarder_route_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    await logToolCall("galyarder_route_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    await logToolCall("galyarder_route_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    if (isProviderSpecific) {
      ...
      // GalyarderRoute's global /v1/models is always a cached/local catalog
    }
    ...
    await logToolCall(
      "galyarder_route_list_models_catalog",
      args,
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    await logToolCall("galyarder_route_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("galyarder_route_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
/**
 * Create and configure the GalyarderRoute MCP Server with all essential tools.
 */
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Returns GalyarderRoute health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("galyarder_route_get_health", async (args) => {
  ...
  server.registerTool(
    "galyarder_route_route_request",
    {
      description: "Sends a chat completion request through GalyarderRoute intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("galyarder_route_route_request", (args) =>
      handleRouteRequest(routeRequestInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_cost_report",
    {
      description: "Generates a cost report for the specified period",
      inputSchema: costReportInput,
    },
    withScopeEnforcement("galyarder_route_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("galyarder_route_list_models_catalog", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("galyarder_route_set_resilience_profile", (args) =>
      handleSetResilienceProfile(setResilienceProfileInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_test_combo",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_test_combo", (args) =>
      handleTestCombo(testComboInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("galyarder_route_explain_route", (args) =>
      handleExplainRoute(explainRouteInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_get_session_snapshot",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "galyarder_route_db_health_check",
    {
      description:
        "Diagnoses or repairs GalyarderRoute database drift, including broken combo references and orphan quota/domain rows",
      inputSchema: dbHealthCheckInput,
    },
    withScopeEnforcement("galyarder_route_db_health_check", (args) =>
      handleDbHealthCheck(dbHealthCheckInput.parse(args ?? {}))
    )
  );

  server.registerTool(
    "galyarder_route_sync_pricing",
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into GalyarderRoute without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("galyarder_route_sync_pricing", (args) =>
      handleSyncPricing(syncPricingInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_web_search",
    {
      description:
        "Performs a web search using GalyarderRoute's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("galyarder_route_web_search", (args) =>
      handleWebSearch(webSearchInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_cache_stats",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_cache_stats", () => handleCacheStats())
  );

  server.registerTool(
    "galyarder_route_cache_flush",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_cache_flush", (args) =>
  ...
  server.registerTool(
    "galyarder_route_oneproxy_fetch",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_oneproxy_fetch", (args) =>
      handleOneproxyFetch(oneproxyFetchInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_oneproxy_rotate",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_oneproxy_rotate", (args) =>
      handleOneproxyRotate(oneproxyRotateInput.parse(args))
    )
  );

  server.registerTool(
    "galyarder_route_oneproxy_stats",
    {
      description:
    ...
    withScopeEnforcement("galyarder_route_oneproxy_stats", (args) =>
 ...
 * Called when `galyarder-route --mcp` is used.
 */
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] GalyarderRoute MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] GalyarderRoute MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
