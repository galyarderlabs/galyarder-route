...
function getDataDir(): string {
  return process.env.DATA_DIR || path.join(os.homedir(), ".galyarder-route");
}
...
function projectFiltersTrusted(
  ...
  if (process.env.GALYARDER_ROUTE_RTK_TRUST_PROJECT_FILTERS === "1") return true;
...
export function loadRtkFilters(options: RtkFilterLoadOptions = {}): RtkFilterDefinition[] {
  const currentCacheKey = [
    ...
    process.env.GALYARDER_ROUTE_RTK_TRUST_PROJECT_FILTERS === "1" ? "env-trust" : "env-normal",
  ].join("|");
