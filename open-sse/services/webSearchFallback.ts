...
export const GALYARDER_ROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME = "galyarder_route_web_search";
...
function buildFallbackTool(tool: JsonRecord): JsonRecord {
  return {
    type: "function",
    function: {
      name: GALYARDER_ROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME,
...
export function prepareWebSearchFallbackBody<T extends JsonRecord>(
  ...
  if (!toolNames.has(GALYARDER_ROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = {
      type: "function",
      function: { name: GALYARDER_ROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME },
    } as T["tool_choice"];
  }
