...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.GALYARDER_ROUTE_QODER_WORKSPACE || ""
  ).trim();
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an GalyarderRoute OpenAI-compatible request through the Qoder CLI transport.",
...
function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "galyarder-route.user@qoder.sh";
  const userInfo = {
    uid: uid,
