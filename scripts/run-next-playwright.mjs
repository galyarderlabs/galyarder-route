...
const bootstrapMode = process.env.GALYARDER_ROUTE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.GALYARDER_ROUTE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "galyarder-route-e2e-password";
const testServerEnv = {
  ...sanitizeColorEnv(bootstrapEnvVars),
  ...sanitizeColorEnv(process.env),
  DATA_DIR: playwrightDataDir,
  NEXT_PUBLIC_GALYARDER_ROUTE_E2E_MODE: process.env.NEXT_PUBLIC_GALYARDER_ROUTE_E2E_MODE || "1",
  GALYARDER_ROUTE_DISABLE_BACKGROUND_SERVICES:
    process.env.GALYARDER_ROUTE_DISABLE_BACKGROUND_SERVICES || "true",
  GALYARDER_ROUTE_DISABLE_TOKEN_HEALTHCHECK: process.env.GALYARDER_ROUTE_DISABLE_TOKEN_HEALTHCHECK || "true",
  GALYARDER_ROUTE_DISABLE_LOCAL_HEALTHCHECK: process.env.GALYARDER_ROUTE_DISABLE_LOCAL_HEALTHCHECK || "true",
  GALYARDER_ROUTE_HIDE_HEALTHCHECK_LOGS: process.env.GALYARDER_ROUTE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        GALYARDER_ROUTE_E2E_PASSWORD: "",
        GALYARDER_ROUTE_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        GALYARDER_ROUTE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.GALYARDER_ROUTE_USE_TURBOPACK
    ? {
        GALYARDER_ROUTE_USE_TURBOPACK: process.env.GALYARDER_ROUTE_USE_TURBOPACK,
      }
    : {}),
};

export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  return mode === "dev" && env.GALYARDER_ROUTE_USE_TURBOPACK !== "1";
}
...
async function runBuildForStart() {
  ...
  if (process.env.GALYARDER_ROUTE_PLAYWRIGHT_SKIP_BUILD === "1") return;
