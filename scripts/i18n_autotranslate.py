#!/usr/bin/env python3
"""
Galyarder Route i18n Auto-Translator
This script scans all docs/i18n directory markdown files and uses an LLM
API (like Galyarder Route itself) to translate any English paragraphs into the
target language.

Usage:
  python3 scripts/i18n_autotranslate.py --api-url http://localhost:20128/v1 --api-key sk-your-galyarder-route-key --model cx/gpt-5.4
"""
...
def translate_block(text, target_language, api_url, api_key, model):
    ...
    prompt = (
        f"You are a professional technical translator working on the Galyarder Route proxy project documentation.\n"
...
def main():
    parser = argparse.ArgumentParser(description="Galyarder Route Auto-Translator for i18n Markdown")
    parser.add_argument("--api-url", default="http://localhost:20128/v1", help="Base URL of Galyarder Route or target provider")
