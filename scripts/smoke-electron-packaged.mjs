...
export const LINUX_EXECUTABLE_NAMES = ["galyarder-route-desktop", "galyarder-route", "GalyarderRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "GalyarderRoute.app", "Contents", "MacOS", "GalyarderRoute"),
    join(distDir, "mac-arm64", "GalyarderRoute.app", "Contents", "MacOS", "GalyarderRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "GalyarderRoute.exe"),
    join(distDir, "win-x64-unpacked", "GalyarderRoute.exe"),
    join(distDir, "win-arm64-unpacked", "GalyarderRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["GalyarderRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing GalyarderRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["galyarder-route-desktop", "GalyarderRoute", "galyarder-route"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged GalyarderRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "galyarder-route-electron-smoke-")));
  const removeDataDir =
