...
import { createGalyarderRouteWsBridge } from "./v1-ws-bridge.mjs";
...
if (fs.existsSync(rootAppDir) && fs.statSync(rootAppDir).isDirectory()) {
  ...
  console.error("Please rename or delete the root 'app/' directory before starting GalyarderRoute.\n");
  process.exit(1);
}
...
const useTurbopack = dev && mergedEnv.GALYARDER_ROUTE_USE_TURBOPACK === "1";
process.env.GALYARDER_ROUTE_WS_BRIDGE_SECRET ||= randomUUID();
...
async function start() {
  ...
  const responsesWsProxy = createResponsesWsProxy({
    ...
    bridgeSecret: process.env.GALYARDER_ROUTE_WS_BRIDGE_SECRET,
  });
  const wsBridge = createGalyarderRouteWsBridge({
    baseUrl: `http://127.0.0.1:${dashboardPort}`,
  });
