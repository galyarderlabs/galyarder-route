#!/usr/bin/env node
/**
 * system-info.mjs — GalyarderRoute System Information Reporter (#280)
 *
...
lines.push("GalyarderRoute System Information Report");
...
lines.push(section("GalyarderRoute"));
try {
...
const installedGlobal = run("npm list -g galyarder-route --depth=0 2>/dev/null | grep galyarder-route");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep galyarder-route | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/galyarderlabs/galyarder-route/issues`
);
