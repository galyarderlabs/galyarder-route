...
const backupRoot = path.join(os.tmpdir(), `galyarder-route-build-isolated-${process.pid}-${Date.now()}`);
...
export function getTransientBuildPaths(rootDir = projectRoot, env = process.env) {
  ...
  if (env.GALYARDER_ROUTE_BUILD_MOVE_TASKS === "1") {
    paths.push({
...
export function resolveNextBuildBundlerFlag(baseEnv = process.env) {
  return baseEnv.GALYARDER_ROUTE_USE_TURBOPACK === "1" ? "--turbopack" : "--webpack";
}
