...
/**
 * GalyarderRoute — Prepublish Build Script
 *
...
console.log("🔨 GalyarderRoute — Building for npm publish...\n");
...
const nextBuildBundlerFlag =
  process.env.GALYARDER_ROUTE_USE_TURBOPACK === "1" ? "--turbopack" : "--webpack";
...
const cliSrcFile = join(ROOT, "bin", "galyarder-route.ts");
const cliDestFile = join(ROOT, "bin", "galyarder-route.mjs");
...
if (existsSync(cliSrcFile)) {
  ...
  try {
    execFileSync(
      ...
      [
        "esbuild",
        "bin/galyarder-route.ts",
        "--bundle",
        "--platform=node",
        "--packages=external",
        "--format=esm",
        "--outfile=bin/galyarder-route.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/galyarder-route.mjs");
  } catch (err: any) {
