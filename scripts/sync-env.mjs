#!/usr/bin/env node
/**
 * GalyarderRoute — Environment Sync
 *
...
const CRYPTO_SECRETS = {
  ...
  MACHINE_ID_SALT: () => `galyarder-route-${randomBytes(8).toString("hex")}`,
};
 ...
 * @see https://github.com/galyarderlabs/galyarder-route/issues/1622
 */
...
function resolveDataDir(env = process.env) {
  ...
  if (xdg) return join(resolve(xdg), "galyarder-route");
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.galyarder-route/server.env, ~/.galyarder-route/.env, ` +
