...
async function runAutomatedChecks() {
  ...
  for (const code of legacyCheckLocales) {
    ...
    try {
      ...
      if (content.includes("**[English](#-galyarder-route--the-free-ai-gateway)**")) {
        anchorLineRemoved = false;
      }
      if (content.includes("## 🇧🇷 GalyarderRoute")) {
...
async function main() {
  ...
  automatedChecksLines.push(
    ...
    `- Apêndice "## 🇧🇷 GalyarderRoute" removido em ja/ar: **${automated.brAppendixRemoved ? "OK" : "PENDENTE"}**`,
