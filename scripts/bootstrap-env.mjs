#!/usr/bin/env node
/**
 * GalyarderRoute — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.galyarder-route/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by GalyarderRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/galyarder-route.mjs`.
  ...
  if (needsPersist) {
    merged.GALYARDER_ROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "galyarder-route-field-encryption-v1", 32)
              const legacySalt = "galyarder-route-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.galyarder-route/server.env or ~/.galyarder-route/.env,"
                );
                log(
                  "   or run: galyarder-route reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
