...
const uninstallAlreadyInProgress =
  process.env.GALYARDER_ROUTE_SKIP_UNINSTALL_HOOK === "1" ||
  process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 GalyarderRoute Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete galyarder-route 2>/dev/null", { stdio: "ignore" });
} catch {
...
const dataDir = process.env.DATA_DIR || path.join(os.homedir(), ".galyarder-route");
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g galyarder-route", {
      stdio: "inherit",
      env: {
        ...process.env,
        GALYARDER_ROUTE_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ GalyarderRoute has been successfully uninstalled from your system.");
    if (!fullUninstall) {
