...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://galyarder-route.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://galyarder-route.local") {
    return null;
  }
...
function getForwardHeaders(requestUrl, requestHeaders) {
  ...
  if (isText(authorization)) {
    ...
    const url = new URL(requestUrl, "http://galyarder-route.local");
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-galyarder-route-ws-session-id": this.sessionId,
      "x-galyarder-route-ws-request-id": requestId,
    };
...
export function createGalyarderRouteWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createGalyarderRouteWsBridge requires a baseUrl");
  }
