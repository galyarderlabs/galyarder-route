...
function getDataDir() {
  ...
  return path.join(os.homedir(), ".galyarder-route");
}
...
const ROUTER_BASE_URL = (
  process.env.GALYARDER_ROUTE_BASE_URL ||
...
async function intercept(req, res, bodyBuffer, mappedModel) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`GalyarderRoute ${response.status}: ${errText}`);
    }
...
const server = https.createServer(sslOptions, async (req, res) => {
  ...
  // Anti-loop: requests from GalyarderRoute bypass interception
  if (req.headers["x-galyarder-route-source"] === "galyarder-route") {
