 ...
 * To route Kiro's traffic through GalyarderRoute, we intercept it using MITM,
 ...
 *
 * To use: Install GalyarderRoute's MITM certificate, then run:
 *   galyarder-route mitm start --targets kiro
 *
 * The MITM server intercepts requests to api.anthropic.com and forwards
 * them to the GalyarderRoute proxy (localhost:20128) instead.
 */
...
export const KIRO_MITM_PROFILE: MitmTarget = {
  ...
  name: "Kiro IDE",
  description:
    "Intercepts Kiro IDE requests to api.anthropic.com and routes them through GalyarderRoute.",
  ...
  instructions: [
    "1. Install GalyarderRoute's root certificate: run `galyarder-route cert install` or go to Settings → MITM Certificates",
    "2. Start the MITM proxy: `galyarder-route mitm start --target kiro`",
    "3. Set your system HTTP proxy to 127.0.0.1:20130 (or use transparent MITM via DNS override)",
    "4. Open Kiro IDE — API calls will be automatically routed through GalyarderRoute.",
    "5. Verify: check the Proxy Logs in GalyarderRoute dashboard and look for provider=anthropic source=mitm",
  ],
