...
import { isLocalProvider } from "@galyarder-route/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS } from "@galyarder-route/open-sse/config/constants.ts";
import { preflightQuota } from "@galyarder-route/open-sse/services/quotaPreflight.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@galyarder-route/open-sse/services/errorClassifier.ts";
import { getCodexModelScope } from "@galyarder-route/open-sse/executors/codex.ts";
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-galyarder-route-session"),
    "header"
  );
 ...
 * Feature #1350: Supports GALYARDER_ROUTE_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.GALYARDER_ROUTE_API_KEY || process.env.ROUTER_API_KEY;
