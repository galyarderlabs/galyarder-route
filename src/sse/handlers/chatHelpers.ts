...
import {
  ...
  getTargetFormat,
} from "@galyarder-route/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@galyarder-route/open-sse/config/providerModels.ts";
import { handleChatCore } from "@galyarder-route/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@galyarder-route/open-sse/utils/error.ts";
import { HTTP_STATUS } from "@galyarder-route/open-sse/config/constants.ts";
import {
  runWithProxyContext,
  runWithTlsTracking,
  isTlsFingerprintActive,
} from "@galyarder-route/open-sse/utils/proxyFetch.ts";
import { resolveProxyForConnection } from "@/lib/localDb";
import { CircuitBreakerOpenError, getCircuitBreaker } from "../../shared/utils/circuitBreaker";
import { logProxyEvent } from "../../lib/proxyLogger";
import { logTranslationEvent } from "../../lib/translatorEvents";
import { getRuntimeProviderProfile } from "@galyarder-route/open-sse/services/accountFallback.ts";
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-Galyarder-Route-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-Galyarder-Route-Session-Id", sessionId);
    return cloned;
  }
}
