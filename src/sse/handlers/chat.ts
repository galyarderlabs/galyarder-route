...
} from "@galyarder-route/open-sse/services/accountFallback.ts";
import { getModelInfo, getComboForModel } from "../services/model";
import { errorResponse } from "@galyarder-route/open-sse/utils/error.ts";
import { handleComboChat } from "@galyarder-route/open-sse/services/combo.ts";
import { resolveComboConfig } from "@galyarder-route/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@galyarder-route/open-sse/services/contextHandoff.ts";
import { HTTP_STATUS } from "@galyarder-route/open-sse/config/constants.ts";
import { getTargetFormat } from "@galyarder-route/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@galyarder-route/open-sse/config/providerModels.ts";
...
} from "@galyarder-route/open-sse/services/taskAwareRouter.ts";
import {
...
} from "@galyarder-route/open-sse/services/sessionManager.ts";
import { startQuotaMonitor } from "@galyarder-route/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@galyarder-route/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@galyarder-route/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@galyarder-route/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@galyarder-route/open-sse/services/crofUsageFetcher.ts";
import {
...
export async function handleChat(request: any, clientRawRequest: any = null) {
  ...
  const requestedConnectionId = request.headers.get("x-galyarder-route-connection")?.trim() || null;
  if (sessionId) {
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._galyarderRouteSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._galyarderRouteInternalRequest !== "context-handoff") {
