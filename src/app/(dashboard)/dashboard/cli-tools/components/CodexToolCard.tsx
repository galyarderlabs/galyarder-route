...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_galyarder_route for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_galyarder_route" : "<YOUR_GALYARDER_ROUTE_API_KEY>";

    let configContent = `# GalyarderRoute Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
    ...
    if (wireApi === "responses") {
      configContent += `
model_provider = "galyarder-route"

[model_providers.galyarder-route]
name = "GalyarderRoute"
...
# Utilize the built-in OpenAI provider pointed to GalyarderRoute
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!codexStatus.hasGalyarderRoute}
                  loading={restoring}
                >
