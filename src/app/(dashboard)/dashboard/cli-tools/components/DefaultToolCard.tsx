...
export default function DefaultToolCard({
  ...
  const supportsImageModel = tool.supportsImageModel === true;
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk_galyarder_route" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const savedModel = localStorage.getItem(`galyarder-route-cli-model-${toolId}`);
    if (savedModel) {
      if (isMultiModelTool) {
        try {
    ...
    const savedKey = localStorage.getItem(`galyarder-route-cli-key-${toolId}`);
    ...
    if (supportsImageModel) {
      const savedImageModel = localStorage.getItem(`galyarder-route-cli-image-model-${toolId}`);
      if (savedImageModel) setImageModel(savedImageModel);
    }
  }, [toolId, apiKeys, isMultiModelTool, supportsImageModel]);

  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`galyarder-route-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`galyarder-route-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`galyarder-route-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`galyarder-route-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`galyarder-route-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
  ...
  const handleImageModelChange = useCallback(
    (value) => {
      ...
      if (normalized) {
        localStorage.setItem(`galyarder-route-cli-image-model-${toolId}`, normalized);
      } else {
        localStorage.removeItem(`galyarder-route-cli-image-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const replaceVars = useCallback(
    (text) => {
      ...
      return text
        ...
        .replace(/\{\{model\}\}/g, getSelectedModels()[0] || t("modelPlaceholder"))
        .replace(
          /\{\{fallbackModelsJson\}\}/g,
          JSON.stringify(getSelectedModels().slice(1), null, 2)
        )
        .replace(/\{\{imageModel\}\}/g, imageModel || "cx/gpt-5.5");
    },
    [baseUrlWithV1, getSelectedModels, imageModel, resolveApiKeyValue, t]
  );
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/cli-tools/guide-settings/${toolId}`, {
        ...
        body: JSON.stringify({
          baseUrl: baseUrlWithV1,
          apiKey: !cloudEnabled ? "sk_galyarder_route" : null,
          keyId: selectedKeyId,
          model: modelValue,
          models: isMultiModelTool ? getSelectedModels() : undefined,
          imageModel: supportsImageModel ? imageModel || undefined : undefined,
          modelLabels: getSelectedModelLabelMap(),
        }),
      });
  ...
  const supportsDirectSave = ["continue", "opencode", "qwen", "hermes", "g-agent"].includes(toolId);
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk_galyarder_route"}
          </span>
        )}
      </div>
    );
  ...
  const renderImageModelSelector = () => {
    if (!supportsImageModel) return null;
    ...
    return (
      <div className="mt-2 flex items-center gap-2">
        ...
        {imageModel && (
          <>
            <button
              onClick={() => handleCopy(imageModel, "imageModel")}
              className="shrink-0 px-3 py-2 bg-bg-secondary hover:bg-bg-tertiary rounded-lg border border-border transition-colors"
            >
              <span className="material-symbols-outlined text-lg">
                {copiedField === "imageModel" ? "check" : "content_copy"}
              </span>
            </button>
            <button
              onClick={() => handleImageModelChange("")}
              className="p-2 text-text-muted hover:text-red-500 rounded transition-colors"
              title={t("clear")}
            >
              <span className="material-symbols-outlined text-lg">close</span>
          ...
          tool.guideSteps.map((item) => (
            <div key={item.step} className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.type === "imageModelSelector" && renderImageModelSelector()}
                {item.value && (
                  <div className="mt-2 flex items-center gap-2">
