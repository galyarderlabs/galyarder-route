 ...
 *
 * Feature request: https://github.com/galyarderlabs/galyarder-route/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("galyarder-route-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("galyarder-route-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "galyarder-route-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "GalyarderRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.galyarder-route}`,
      models,
    };
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("galyarder-route-cli-key-copilot", value);
  };
