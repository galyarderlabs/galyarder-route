...
function TransportSelector({
  ...
  const options: { value: McpTransport; label: string; desc: string }[] = [
    { value: "stdio", label: "stdio", desc: "Local — IDE spawns process via galyarder-route --mcp" },
...
export default function EndpointPage() {
  const [activeTab, setActiveTab] = useState("route-gateway");
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center gap-3">
        <SegmentedControl
          options={[
            { value: "route-gateway", label: t("tabProxy"), icon: "api" },
      ...
      {activeTab === "route-gateway" && <EndpointPageClient machineId="" />}
