  ...
  try {
    ...
    if (importable.length === 0) {
      if (credentials.length > 0) {
        console.warn(
          `[Zed Import] Found ${credentials.length} keychain credential(s), but none mapped to supported GalyarderRoute providers`
        );
      }
      return NextResponse.json({
        success: true,
    ...
    // Save to database using GalyarderRoute's provider schema
    let savedCount = 0;
