...
// Required tables that must exist in a valid GalyarderRoute database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `galyarder-route-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid GalyarderRoute database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
