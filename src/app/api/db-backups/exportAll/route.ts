...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `galyarder-route-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `galyarder-route-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "galyarder-route-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="galyarder-route-full-backup-${timestamp}.tar.gz"`,
