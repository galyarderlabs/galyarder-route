...
async function getLatestNpmVersion(): Promise<string | null> {
  try {
    const { stdout } = await execFileAsync("npm", ["info", "galyarder-route", "version", "--json"], {
      timeout: 10000,
    });
...
export async function POST(req: NextRequest) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          try {
            await execFileAsync("pm2", ["restart", "galyarder-route", "--update-env"], {
              timeout: 30_000,
              cwd: process.cwd(),
            });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        // Step 1: Install
        send({ step: "install", status: "running", message: `Installing galyarder-route@${latest}...` });
        await execFileAsync(
          "npm",
          ["install", "-g", `galyarder-route@${latest}`, "--ignore-scripts", "--legacy-peer-deps"],
          {
            timeout: 300000,
          }
        );
        send({ step: "install", status: "done", message: `Installed galyarder-route@${latest}` });
        ...
        const routePath = `${globalRoot}/galyarder-route/app`;
        ...
        try {
          await execFileAsync("pm2", ["restart", "galyarder-route", "--update-env"], { timeout: 30000 });
          send({ step: "restart", status: "done", message: "Service restarted" });
        } catch {
