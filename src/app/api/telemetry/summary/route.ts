...
export async function GET(request) {
  try {
    ...
    const { getQuotaMonitorSummary } = await import("@galyarder-route/open-sse/services/quotaMonitor.ts");
    const { getActiveSessions } = await import("@galyarder-route/open-sse/services/sessionManager.ts");
