...
import { z } from "zod";
import { CodexExecutor } from "@galyarder-route/open-sse/executors/codex.ts";
...
function getBridgeSecret(): string {
  return process.env.GALYARDER_ROUTE_WS_BRIDGE_SECRET || "";
}
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-galyarder-route-ws-bridge-secret") || "";
