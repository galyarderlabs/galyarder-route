...
export async function GET() {
  try {
    ...
    const { getAllRateLimitStatus, getLearnedLimits } =
      await import("@galyarder-route/open-sse/services/rateLimitManager");
    const { getAllModelLockouts } = await import("@galyarder-route/open-sse/services/accountFallback");
    const { getInflightCount } = await import("@galyarder-route/open-sse/services/requestDedup.ts");
    const { getQuotaMonitorSummary, getQuotaMonitorSnapshots } =
      await import("@galyarder-route/open-sse/services/quotaMonitor.ts");
    const { getActiveSessions, getAllActiveSessionCountsByKey } =
      await import("@galyarder-route/open-sse/services/sessionManager.ts");
