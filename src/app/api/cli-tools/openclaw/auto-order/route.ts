...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [healthRes, combosRes] = await Promise.allSettled([
      fetch(`${GALYARDER_ROUTE_BASE_URL}/api/monitoring/health`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${GALYARDER_ROUTE_BASE_URL}/api/combos`, { signal: AbortSignal.timeout(5000) }),
    ]);
    ...
    return NextResponse.json({
      ...
      source: "galyarder-route-auto-combo",
    });
  } catch {
    return NextResponse.json({
      provider: {
      ...
      generated_at: new Date().toISOString(),
      source: "galyarder-route-fallback",
    });
  }
}
