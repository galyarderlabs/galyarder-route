 ...
 * Currently supports: continue, opencode, qwen, hermes, g-agent
 */
export async function POST(request, { params }) {
  ...
  const { baseUrl, model, models, imageModel, modelLabels } = validation.data;
  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the GalyarderRoute settings there.
      ...
      case "g-agent":
        return await saveGAgentConfig({ baseUrl, apiKey, model, models, imageModel });
      default:
        return NextResponse.json(
...
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const normalizedBaseUrl = String(baseUrl || "")
    .trim()
    .replace(/\/+$/, "");
  const resolvedModel = model || "cx/gpt-5.5";

  let existingConfig: Record<string, any> = {};
  try {
    const raw = await fs.readFile(configPath, "utf-8");
    existingConfig = (yaml.load(raw) as Record<string, any>) || {};
  } catch {
    // Missing or invalid YAML — start fresh.
  }
  ...
  existingConfig.model = {
    ...
    api_key: apiKey || "sk_galyarder_route",
    api_mode: "chat_completions",
  };

  await fs.writeFile(configPath, yaml.dump(existingConfig, { lineWidth: 120 }), "utf-8");
...
async function saveGAgentConfig({ baseUrl, apiKey, model, models, imageModel }) {
  const home = os.homedir();
  const configPath = path.join(home, ".g-agent", "config.json");
  ...
  const normalizedBaseUrl = String(baseUrl || "")
    .trim()
    .replace(/\/+$/, "");
  const apiBase = normalizedBaseUrl.endsWith("/v1")
    ? normalizedBaseUrl
    : `${normalizedBaseUrl}/v1`;
  const selectedModels = Array.isArray(models)
    ? [...new Set(models.map((value) => String(value || "").trim()).filter(Boolean))]
    : [];
  const primaryModel = String(model || selectedModels[0] || "cx/gpt-5.5").trim();
  const fallbackModels = selectedModels.filter((value) => value && value !== primaryModel);
  const selectedImageModel = String(imageModel || "cx/gpt-5.5").trim();
  ...
  existingConfig.providers = {
    ...(existingConfig.providers || {}),
    proxy: {
      ...(existingConfig.providers?.proxy || {}),
      type: "openai-compatible",
      apiBase,
      apiKey: apiKey || "sk_galyarder_route",
    },
  };
  existingConfig.agents = {
    ...
    defaults: {
      ...
      model: primaryModel,
      routing: {
        ...(existingConfig.agents?.defaults?.routing || {}),
        mode: "proxy",
        proxyProvider: "proxy",
        fallbackModels,
      },
  ...
  existingConfig.visual = {
    ...(existingConfig.visual || {}),
    enabled: true,
    imageGen: {
      ...(existingConfig.visual?.imageGen || {}),
      provider: "openai-compatible",
      apiBase,
      apiKey: apiKey || "sk_galyarder_route",
      model: selectedImageModel,
      timeout: existingConfig.visual?.imageGen?.timeout || 180,
    },
  };
  ...
  return NextResponse.json({
    success: true,
    message: `G-Agent config saved to ${configPath}`,
    configPath,
    primaryModel,
    fallbackModels,
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the GalyarderRoute model entry
  const normalizedBaseUrl = String(baseUrl || "")
    .trim()
    .replace(/\/+$/, "");
  const routerModel = {
...
async function saveQwenConfig({ baseUrl, apiKey, model }) {
  ...
  const normalizedBaseUrl = String(baseUrl || "")
    .trim()
    .replace(/\/+$/, "");
  const resolvedApiKey = apiKey || "sk_galyarder_route";
