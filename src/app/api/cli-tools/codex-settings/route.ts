...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.galyarder-route]
...
// Check if config has GalyarderRoute settings
const hasGalyarderRouteConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "galyarder-route"') ||
    config.includes("[model_providers.galyarder-route]")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasGalyarderRoute: hasGalyarderRouteConfig(config),
...
// POST - Update GalyarderRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only GalyarderRoute related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use GALYARDER_ROUTE_API_KEY
    parsed._root.model_provider = "galyarder-route";
    parsed._sections["model_providers.galyarder-route"] = {
      name: "GalyarderRoute",
...
// DELETE - Remove GalyarderRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove GalyarderRoute related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "galyarder-route") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove galyarder-route provider section
    delete parsed._sections["model_providers.galyarder-route"];
    ...
    return NextResponse.json({
      success: true,
      message: "GalyarderRoute settings removed successfully",
    });
  } catch (error) {
