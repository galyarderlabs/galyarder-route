...
// Check if GalyarderRoute OpenAI-compatible provider is configured
const hasGalyarderRouteConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["galyarder-route"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("galyarder-route")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasGalyarderRoute: hasGalyarderRouteConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use GalyarderRoute as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update GalyarderRoute as openai-compatible provider
...
// DELETE - Remove GalyarderRoute config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove GalyarderRoute provider
    delete auth["openai-compatible"];
    delete auth["galyarder-route"];
    ...
    return NextResponse.json({
      success: true,
      message: "GalyarderRoute settings removed from Kilo Code",
    });
  } catch (error) {
