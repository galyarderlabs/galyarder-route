...
// Check if a tool has GalyarderRoute configured by reading its config file directly
...
async function checkToolConfigStatus(toolId: string): Promise<string> {
  try {
    ...
    if (toolId === "codex") {
      ...
      const hasGalyarderRoute =
        lower.includes("galyarder-route") ||
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      if (!hasGalyarderRoute) return "not_configured";
    ...
    switch (toolId) {
      ...
      case "cline":
      case "kilo":
        // Generic check: look for GalyarderRoute-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("galyarder-route") ||
          configStr.includes("sk_galyarder_route") ||
