...
// Check if settings has GalyarderRoute customModels
const hasGalyarderRouteConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some((m) => m.id === "custom:GalyarderRoute-0");
};
...
// POST - Update GalyarderRoute customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove existing GalyarderRoute config if any
    settings.customModels = settings.customModels.filter((m) => m.id !== "custom:GalyarderRoute-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new GalyarderRoute config
    const customModel = {
      model: model,
      id: "custom:GalyarderRoute-0",
      index: 0,
...
// DELETE - Remove GalyarderRoute customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove GalyarderRoute customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => m.id !== "custom:GalyarderRoute-0");
