...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        sessionStorage.setItem("galyarder_route_login_time", String(Date.now()));
        router.push("/dashboard");
        router.refresh();
      } else {
        const data = await res.json();
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-bg p-6">
        ...
        >
          ...
          <p className="text-center text-xs text-text-muted/60 mt-8">
            GalyarderRoute — {t("unifiedAiApiProxy")}
          </p>
        </div>
      </div>
    );
        ...
        >
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-8">
              ...
              <span className="text-xl font-semibold text-text-main tracking-tight">GalyarderRoute</span>
            </div>
