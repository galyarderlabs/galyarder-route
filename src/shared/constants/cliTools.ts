...
import { getClaudeCodeDefaultModels } from "@galyarder-route/open-sse/config/providerRegistry";
...
export const CLI_TOOLS = {
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model galyarder-route/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  hermes: {
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Use the JSON block below as the OpenAI-compatible provider definition for GalyarderRoute.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  "g-agent": {
    ...
    color: "#9B00FF",
    description:
      "G-Agent configured through Galyarder Route. First selected model is primary; the rest are saved as fallback models.",
    docsUrl: "/docs?section=cli-tools&tool=g-agent",
    configType: "guide",
    defaultCommand: "g-agent",
    modelSelectionMode: "multiple",
    supportsImageModel: true,
    guideSteps: [
      {
        step: 1,
        title: "Install G-Agent",
        desc: "Use your existing g-agent binary or install it into PATH.",
      },
      { step: 2, title: "API Key", type: "apiKeySelector" },
      { step: 3, title: "Base URL", value: "{{baseUrl}}", copyable: true },
      { step: 4, title: "Primary + Fallback Models", type: "modelSelector" },
      { step: 5, title: "Image Model", type: "imageModelSelector" },
      {
        step: 6,
        title: "Save Config",
        desc: "Click Save Config below to update ~/.g-agent/config.json. Chat models are saved under agents.defaults; the image model is saved to visual.imageGen.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  amp: {
    ...
    notes: [
      {
        type: "info",
        text: "Use GalyarderRoute model aliases to keep Amp shorthand mappings stable across provider updates.",
      },
      {
        type: "warning",
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Map Amp shorthand names such as g25p or cs45 to GalyarderRoute aliases in your local config.",
      },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description:
      "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via GalyarderRoute",
    ...
    notes: [
      {
        type: "info",
        text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. GalyarderRoute works as an OpenAI-compatible endpoint.",
      },
      {
        type: "info",
        text: "Any model available in GalyarderRoute can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more.",
      },
      {
        type: "warning",
      ...
      {
        type: "error",
        text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use GalyarderRoute with alicode/openrouter/anthropic/gemini providers instead.",
      },
    ],
    modelAliases: [
      "coder-model",
    ...
    codeBlock: {
      language: "json",
      code: `# ~/.qwen/settings.json — GalyarderRoute via security.auth
{
  "security": {
    "auth": {
      "selectedType": "openai",
