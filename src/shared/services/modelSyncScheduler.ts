...
const globalState = globalThis as typeof globalThis & {
  __galyarderRouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__galyarderRouteModelSyncInternalAuthToken || randomUUID();
    globalState.__galyarderRouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__galyarderRouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__galyarderRouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call GalyarderRoute's own API
