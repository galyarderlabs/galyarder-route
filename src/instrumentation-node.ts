...
function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.GALYARDER_ROUTE_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  ...
  await import("@galyarder-route/open-sse/index.ts");
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@galyarder-route/open-sse/services/batchProcessor");
    initBatchProcessor();
