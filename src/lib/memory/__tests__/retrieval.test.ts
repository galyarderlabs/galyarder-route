...
describe("Memory Retrieval — FTS5 integration", () => {
  ...
  beforeEach(() => {
    ...
    savedDb = (globalThis as any).__galyarderRouteDb;
    ...
    setupSchema(db);
    setupFts(db);
    (globalThis as any).__galyarderRouteDb = db;
  });

  afterEach(() => {
    // Restore the previous singleton (or remove it)
    if (savedDb) {
      (globalThis as any).__galyarderRouteDb = savedDb;
    } else {
      delete (globalThis as any).__galyarderRouteDb;
    }
    try {
      db.close();
    } catch {
