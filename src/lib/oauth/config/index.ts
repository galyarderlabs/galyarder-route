 ...
 * with the running GalyarderRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.GALYARDER_ROUTE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.GALYARDER_ROUTE_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.GALYARDER_ROUTE_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
