...
// ─── Provider mapping: LiteLLM provider → GalyarderRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → GalyarderRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (GalyarderRoute format)
 * Filters: only chat/completion modes (skip image/audio/embedding)
 */
export function transformToGalyarderRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to GalyarderRoute providers
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToGalyarderRoute(raw);
