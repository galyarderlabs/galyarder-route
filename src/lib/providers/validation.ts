...
} from "@galyarder-route/open-sse/services/claudeCodeCompatible.ts";
import {
...
import { getGigachatAccessToken } from "@galyarder-route/open-sse/services/gigachatAuth.ts";
import { validateQoderCliPat } from "@galyarder-route/open-sse/services/qoderCli.ts";
import {
...
} from "@galyarder-route/open-sse/config/bedrock.ts";
import {
...
import {
  ...
  buildOciChatUrl,
  buildOciModelsUrl,
} from "@galyarder-route/open-sse/config/oci.ts";
import {
...
} from "@galyarder-route/open-sse/config/runway.ts";
import { PETALS_DEFAULT_MODEL, normalizePetalsBaseUrl } from "@galyarder-route/open-sse/config/petals.ts";
import {
  buildMaritalkChatUrl,
  buildMaritalkModelsUrl,
} from "@galyarder-route/open-sse/config/maritalk.ts";
import { signAwsRequest } from "@galyarder-route/open-sse/utils/awsSigV4.ts";
...
async function validateChatGptWebProvider({ apiKey, providerSpecificData = {} }: any) {
  try {
    ...
    const { tlsFetchChatGpt, TlsClientUnavailableError } =
      await import("@galyarder-route/open-sse/services/chatgptTlsClient.ts");
    ...
    vertex: async ({ apiKey }: any) => {
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@galyarder-route/open-sse/executors/vertex.ts");
