...
export function getAutoUpdateConfig(env: NodeJS.ProcessEnv = process.env): AutoUpdateConfig {
  const dataDir = env.DATA_DIR || "/tmp/galyarder-route";
  const repoDir = env.AUTO_UPDATE_REPO_DIR || "/workspace/galyarder-route";
  ...
  return {
    ...
    composeService: env.AUTO_UPDATE_SERVICE || "galyarder-route-cli",
...
export async function validateAutoUpdateRuntime(
  ...
  if (!(await existsImpl("/var/run/docker.sock"))) {
    return {
      supported: false,
      reason: "Docker socket is not mounted into the GalyarderRoute container.",
      composeCommand: null,
    };
  }
  ...
  try {
    ...
    return {
      supported: false,
      reason: "git is not available inside the GalyarderRoute container.",
      composeCommand: null,
    };
  }
  ...
  if (!composeCommand) {
    return {
      supported: false,
      reason:
        "Neither docker compose nor docker-compose is available inside the GalyarderRoute container.",
      composeCommand: null,
    };
  }
...
export function buildNpmUpdateScript(latest: string): string {
  return [
    "set -eu",
    `npm install -g galyarder-route@${latest} --ignore-scripts --legacy-peer-deps`,
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart galyarder-route || true",
    "fi",
...
export function buildSourceUpdateScript(latest: string, gitRemote = "origin"): string {
  ...
  return [
    ...
    "npm run build",
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart galyarder-route --update-env || true",
    "fi",
