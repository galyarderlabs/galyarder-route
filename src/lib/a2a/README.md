# Galyarder Route A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use Galyarder Route as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes Galyarder Route as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
...
└──────────────────────────────────────────────────────────────────┘
                       │
                       ▼  Galyarder Route Gateway (internal)
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through Galyarder Route intelligent pipeline",
...
Routes prompts through Galyarder Route's intelligent pipeline with full observability.
...
Discovers Galyarder Route agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use Galyarder Route A2A as a custom LLM.
"""
...
class Galyarder RouteA2A(BaseLLM):
...
An orchestrator agent delegates code generation to Galyarder Route, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via Galyarder Route A2A
    ...
    # Step 2: Review the code via Galyarder Route A2A (different model)
...
Multiple agents share quota through Galyarder Route, using the quota skill to coordinate.
...
```
Authorization: Bearer YOUR_GALYARDER_ROUTE_API_KEY
```

If no API key is configured on the server (`GALYARDER_ROUTE_API_KEY` is empty), authentication is bypassed.
