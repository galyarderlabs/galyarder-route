 ...
 * Priority: system daemon socket → GalyarderRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to GalyarderRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `galyarder-route-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
    ...
    const hostname = os
      ...
      .replace(/[^a-zA-Z0-9-]/g, "-")
      .toLowerCase();
    return hostname || "galyarder-route";
  }
}
