...
class SandboxRunner {
  ...
  async run(
    ...
    const dockerArgs = [
      ...
      "--rm",
      "--name",
      `galyarder-route-sandbox-${sandboxId}`,
      "--cpus",
  ...
  kill(sandboxId: string): boolean {
    ...
    if (proc) {
      proc.kill("SIGTERM");
      this.runningContainers.delete(sandboxId);
      childProcess.spawn("docker", ["kill", `galyarder-route-sandbox-${sandboxId}`], {
        stdio: "ignore",
      });
      return true;
    }
  ...
  killAll(): void {
    for (const [id, proc] of this.runningContainers) {
      proc.kill("SIGTERM");
      childProcess.spawn("docker", ["kill", `galyarder-route-sandbox-${id}`], { stdio: "ignore" });
    }
