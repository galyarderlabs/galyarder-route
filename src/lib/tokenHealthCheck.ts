...
} from "@galyarder-route/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("GALYARDER_ROUTE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("GALYARDER_ROUTE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __galyarderRouteTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getHCState() {
  if (!globalThis.__galyarderRouteTokenHC) {
    globalThis.__galyarderRouteTokenHC = { initialized: false, interval: null };
  }
  return globalThis.__galyarderRouteTokenHC;
}
