...
import { PROVIDER_ID_TO_ALIAS } from "@galyarder-route/open-sse/config/providerModels.ts";
...
export async function getPricingForModel(provider: string, model: string) {
  ...
  const { PROVIDER_ID_TO_ALIAS } = await import("@galyarder-route/open-sse/config/providerModels");
