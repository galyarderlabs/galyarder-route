...
function resolveMigrationsDir(): string {
  const configuredDir = process.env.GALYARDER_ROUTE_MIGRATIONS_DIR;
  ...
  throw new Error(
    "[Migration] Could not resolve migrations directory. Set GALYARDER_ROUTE_MIGRATIONS_DIR."
  );
}
...
function ensureMigrationsTable(db: Database.Database): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS _galyarder_route_migrations (
...
function getAppliedVersions(db: Database.Database): Set<string> {
  const rows = db.prepare("SELECT version FROM _galyarder_route_migrations").all() as Array<{
    version: string;
  }>;
...
function getAppliedRecords(db: Database.Database): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _galyarder_route_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    const legacyRow = db
      .prepare("SELECT version, name FROM _galyarder_route_migrations WHERE version = ? AND name = ?")
    ...
    const targetRow = db
      .prepare("SELECT version FROM _galyarder_route_migrations WHERE version = ?")
      .get(compatibility.toVersion) as { version: string } | undefined;

    const applyRepair = db.transaction(() => {
      if (targetRow) {
        db.prepare("DELETE FROM _galyarder_route_migrations WHERE version = ? AND name = ?").run(
          compatibility.fromVersion,
          compatibility.fromName
        );
      } else {
        db.prepare(
          "UPDATE _galyarder_route_migrations SET version = ?, name = ? WHERE version = ? AND name = ?"
        ).run(
    ...
    const residualRow = db
      .prepare("SELECT version, name FROM _galyarder_route_migrations WHERE version = ?")
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _galyarder_route_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
...
function rehomeLegacyVersionSlotMigrations(
  ...
  for (const legacy of LEGACY_VERSION_SLOT_MIGRATIONS) {
    ...
    const applyRepair = db.transaction(() => {
      ...
      db.prepare("UPDATE _galyarder_route_migrations SET version = ? WHERE version = ? AND name = ?").run(
        legacyVersion,
        legacy.version,
        legacy.name
      );
...
export function runMigrations(db: Database.Database, options?: { isNewDb?: boolean }): number {
  ...
  if (mismatches.length > 0) {
    ...
    console.error(
      `[Migration] The version-only tracking will skip these (version already applied), ` +
        `but please report this to the GalyarderRoute maintainers.`
    );
  }
  ...
  // from the _galyarder_route_migrations table.
  ...
  for (const migration of pending) {
    const applyMigration = db.transaction(() => {
      ...
      db.prepare("INSERT INTO _galyarder_route_migrations (version, name) VALUES (?, ?)").run(
        migration.version,
        migration.name
      );
    });
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _galyarder_route_migrations (version, name) VALUES (?, ?)"
...
export function getMigrationStatus(db: Database.Database): {
  ...
  const appliedRows = db
    .prepare("SELECT version, name, applied_at FROM _galyarder_route_migrations ORDER BY version")
