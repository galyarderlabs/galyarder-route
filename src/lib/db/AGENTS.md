...
- **`migrationRunner.ts`** — Applies versioned SQL files from `db/migrations/` inside transactions. Tracks applied migrations in `_galyarder_route_migrations`. Runs at startup; each migration is idempotent.
