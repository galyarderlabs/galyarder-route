...
function createNativeSqliteLoadError(error: unknown): Error {
  ...
  const detail =
    ...
    "Run `npm rebuild better-sqlite3` in the GalyarderRoute project and start again. " +
  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir galyarder-route`
    );
  }
}
...
declare global {
  var __galyarderRouteDb: import("better-sqlite3").Database | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__galyarderRouteDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__galyarderRouteDb = db;
  } else {
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.GALYARDER_ROUTE_FORCE_DB_HEALTHCHECK === "1") return true;
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.GALYARDER_ROUTE_DB_HEALTHCHECK_INTERVAL_MS;
...
export function getDbInstance(): SqliteDatabase {
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _galyarder_route_migrations (version, name)
