...
function getLHCState() {
  if (!globalThis.__galyarderRouteLocalHC) {
    globalThis.__galyarderRouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__galyarderRouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("GALYARDER_ROUTE_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
