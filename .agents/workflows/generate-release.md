...
gh pr create \
  --repo galyarderlabs/galyarder-route \
  --base main \
...
if [ -z "$NOTES" ]; then NOTES="Galyarder Route v$VERSION Release"; fi

git tag -a "v$VERSION" -m "Release v$VERSION"
git push origin "v$VERSION"
gh release create "v$VERSION" --repo galyarderlabs/galyarder-route --title "v$VERSION" --notes "$NOTES" --target main || gh release edit "v$VERSION" --repo galyarderlabs/galyarder-route --title "v$VERSION" --notes "$NOTES"
```
...
gh run list --repo galyarderlabs/galyarder-route --workflow docker-publish.yml --limit 3

# Wait for the Docker build to complete (usually 5–10 min)
gh run watch --repo galyarderlabs/galyarder-route
```
...
2. **Electron Build**
3. **NPM Registry Publish** (Check with `npm info galyarder-route version`)

```bash
# Monitor Docker Hub workflow
gh run list --repo galyarderlabs/galyarder-route --workflow docker-publish.yml --limit 1
gh run watch <RUN_ID>

# Monitor Electron build
gh run list --repo galyarderlabs/galyarder-route --workflow electron-release.yml --limit 1
gh run watch <RUN_ID>

# Verify NPM version
npm info galyarder-route version
```
...
- If necessary, re-trigger the workflow using `gh workflow run <workflow_name.yml> --repo galyarderlabs/galyarder-route --ref v2.x.y`
...
- After npm publish, verify with `npm info galyarder-route version`
- Lock file sync errors are caused by skipping `npm install` after version bump
- Use `gh auth switch -u galyarderlabs` if git push fails with wrong account
